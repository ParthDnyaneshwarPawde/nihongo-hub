...
export default function LectureViewer() {
  ...
  // 🚨 1. GLOBAL THEME SYNC (Reads from LocalStorage directly)
  ...
  if (isLoading) {
    return (
      <div className={`h-screen w-full flex flex-col items-center justify-center font-black uppercase tracking-widest ${isDark ? 'bg-[#0B1121] text-indigo-500' : 'bg-slate-50 text-indigo-600'}`}>
  ...
  const renderNavigation = () => (
    <>
      ...
      <aside className={`fixed lg:relative inset-y-0 left-0 z-50 flex shrink-0 transition-all duration-300 ease-in-out shadow-2xl lg:shadow-none
        ...
        <div className={`w-[300px] lg:w-[320px] h-full flex flex-col border-r shrink-0 ${isDark ? 'bg-[#0B1121] border-slate-800' : 'bg-white/80 backdrop-blur-xl border-slate-200/60 shadow-sm z-10'}`}>
          <div className={`p-6 border-b shrink-0 ${isDark ? 'border-slate-800' : 'border-slate-100/80 bg-gradient-to-b from-slate-50/50 to-transparent'}`}>
            <h2 className={`font-black text-lg mb-2 truncate ${isDark ? 'text-white' : 'text-slate-800'}`}>{courseTitle}</h2>
            <div className="flex items-center gap-3">
              <div className={`flex-1 h-1.5 rounded-full overflow-hidden ${isDark ? 'bg-slate-800' : 'bg-slate-200/80 shadow-inner'}`}>
              ...
              <span className={`text-xs font-black ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>{courseProgress}%</span>
            </div>
          </div>

          <div className={`flex-1 overflow-y-auto custom-scrollbar ${isDark ? '' : 'bg-slate-50/20'}`}>
            {chapters.map((chap, idx) => {
              const isParentLocked = chap.isModLocked || chap.isChapLocked;

              return (
              <div key={chap.id} className={`border-b ${isDark ? 'border-slate-800/50' : 'border-slate-100'}`}>
                
                <button 
                  onClick={() => !isParentLocked && toggleChapter(chap.id)} 
                  className={`w-full p-4 flex items-center justify-between transition-colors 
                    ${isParentLocked ? 'opacity-50 cursor-not-allowed' : isDark ? 'hover:bg-slate-800/50' : 'hover:bg-indigo-50/30 bg-transparent'}`}
                >
                  <div className="text-left flex items-center gap-3">
                    {isParentLocked && <Lock size={16} className="text-rose-500" />}
                    <div>
                      <span className={`text-[10px] font-black uppercase tracking-[0.2em] ${isDark ? 'text-indigo-500' : 'text-indigo-500'}`}>Section {idx + 1}</span>
                      <h3 className={`font-bold text-sm mt-0.5 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>{chap.title}</h3>
                    </div>
                  </div>
                  {!isParentLocked && <ChevronDown size={18} className={`transition-transform duration-300 ${expandedChapters.includes(chap.id) ? 'rotate-180 text-indigo-500' : (isDark ? 'text-slate-500' : 'text-slate-300')}`} />}
                </button>
                ...
                <div className={`overflow-hidden transition-all duration-300 ease-in-out ${expandedChapters.includes(chap.id) ? 'max-h-[800px]' : 'max-h-0'}`}>
                  <div className={`py-2 ${isDark ? 'bg-black/20' : 'bg-slate-50/50 shadow-inner border-y border-slate-100/50'}`}>
                    {chap.items.map(item => {
                      ...
                      return (
                        <button 
                          ...
                          className={`w-full p-3 pl-6 flex items-start gap-3 transition-all text-left
                            ...
                            ${isActive ? (isDark ? 'bg-indigo-500/10 border-l-2 border-indigo-500' : 'bg-gradient-to-r from-indigo-50 to-white border-l-4 border-indigo-600 shadow-sm') : 'border-l-4 border-transparent hover:bg-slate-800/20 dark:hover:bg-slate-800/30 hover:border-slate-200 dark:hover:border-transparent'}`}
                        >
                          <div className="mt-0.5 shrink-0 transition-colors">
                            {isItemLocked ? <Lock size={16} className="text-rose-500" />
                              : isComplete ? <CheckCircle2 size={16} className="text-emerald-500" />
                              : item.type === 'video' ? <PlaySquare size={16} className={isActive ? 'text-indigo-600' : (isDark ? 'text-slate-600' : 'text-slate-400')} />
                              : item.type === 'article' ? <FileText size={16} className={isActive ? 'text-indigo-600' : (isDark ? 'text-slate-600' : 'text-slate-400')} />
                              : <HelpCircle size={16} className={isActive ? 'text-indigo-600' : (isDark ? 'text-slate-600' : 'text-slate-400')} />
                            }
                          </div>
                          <div>
                            <p className={`text-sm font-bold leading-tight transition-colors ${isActive ? (isDark ? 'text-indigo-300' : 'text-indigo-900') : (isDark ? 'text-slate-400' : 'text-slate-600')}`}>{item.title}</p>
                            <p className={`text-[9px] font-black mt-1 uppercase tracking-widest ${isActive ? (isDark ? 'text-indigo-500/70' : 'text-indigo-500') : (isDark ? 'text-slate-600' : 'text-slate-400')}`}>
  ...
  const renderChat = () => (
    <>
      ...
      <aside className={`fixed lg:relative inset-y-0 right-0 z-50 flex shrink-0 transition-all duration-300 ease-in-out shadow-[-20px_0_40px_rgba(0,0,0,0.1)] lg:shadow-none
        ...
        <div className={`w-[300px] lg:w-[320px] h-full flex flex-col border-l shrink-0 ${isDark ? 'bg-[#0B1121] border-slate-800' : 'bg-white/90 backdrop-blur-2xl border-slate-200/60 shadow-[-10px_0_40px_rgba(0,0,0,0.04)]'}`}>
          <div className={`h-16 p-4 border-b flex justify-between items-center shrink-0 ${isDark ? 'border-slate-800 bg-[#151E2E]' : 'border-slate-100/50 bg-gradient-to-r from-slate-50 to-white'}`}>
            <div className={`flex flex-col gap-0.5 ${isDark ? 'text-white' : 'text-slate-800'}`}>
              <div className="font-black flex items-center gap-2 text-sm">
                <MessageSquare size={16} className="text-indigo-500"/> Class Q&A
              </div>
              <span className={`text-[9px] font-bold uppercase tracking-widest truncate max-w-[200px] ${isDark ? 'text-slate-400' : 'text-slate-400'}`}>
                {activeItem ? activeItem.title : 'Loading...'}
              </span>
            </div>
            <button onClick={() => setIsChatOpen(false)} className={`lg:hidden p-2 rounded-xl transition-colors ${isDark ? 'text-slate-400 hover:bg-slate-800' : 'text-slate-400 hover:bg-slate-100'}`}>
              <PanelRightClose size={20}/>
            </button>
          </div>

          <div className={`flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar ${isDark ? '' : 'bg-transparent'}`}>
            {comments.length === 0 ? (
                  ...
                  <div className="flex gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`text-xs font-bold ${comment.isTeacher ? 'text-rose-500 dark:text-rose-400' : isDark ? 'text-slate-300' : 'text-slate-800'}`}>{comment.user}</span>
                      ...
                      <p className={`text-sm font-medium leading-relaxed ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>{comment.text}</p>
                    </div>
                  </div>
                </div>
              ))
          ...
          <div className={`p-4 border-t shrink-0 ${isDark ? 'border-slate-800 bg-[#151E2E]' : 'border-slate-100 bg-white/50'}`}>
            <div className={`relative flex items-center p-2 rounded-2xl border focus-within:border-indigo-500 transition-all shadow-sm
              ${isDark ? 'bg-[#0B1121] border-slate-700' : 'bg-white border-slate-200 focus-within:shadow-md'}`}>
              <input 
                type="text" placeholder="Ask a question..." 
                value={newComment} 
                onChange={(e) => setNewComment(e.target.value)}
                onKeyDown={handleKeyDown}
                className={`flex-1 bg-transparent border-none outline-none px-2 text-sm font-medium placeholder:text-slate-400 ${isDark ? 'text-white' : 'text-slate-800'}`}
              />
              <button onClick={handleSendMessage} className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all ${newComment ? 'bg-gradient-to-br from-indigo-500 to-indigo-600 text-white shadow-md shadow-indigo-600/20 active:scale-95' : (isDark ? 'bg-slate-800 text-slate-500' : 'bg-slate-100 text-slate-400')}`}>
    ...
    return (
      <main className={`flex-1 flex flex-col min-w-0 h-full overflow-hidden transition-all duration-300 ${isDark ? 'bg-[#0B1121]' : 'bg-[#FAFAFA]'}`}>
        <header className={`h-16 px-4 lg:px-6 shrink-0 flex items-center justify-between border-b z-10 transition-colors ${isDark ? 'bg-[#151E2E] border-slate-800' : 'bg-white/80 backdrop-blur-md border-slate-200/60 shadow-sm'}`}>
          <div className="flex items-center gap-3">
            <button onClick={() => setIsNavOpen(!isNavOpen)} className={`p-2 rounded-xl transition-colors ${isDark ? 'hover:bg-slate-800 text-slate-400 hover:text-white' : 'hover:bg-slate-100 text-slate-500 hover:text-slate-900'}`}>
              {isNavOpen ? <PanelLeftClose size={20}/> : <PanelLeftOpen size={20}/>}
            </button>
            <div className={`h-4 w-px mx-1 hidden sm:block ${isDark ? 'bg-slate-700' : 'bg-slate-200'}`}></div>
            <button onClick={() => navigate('/student-dashboard')} className={`text-xs font-black uppercase tracking-widest hidden sm:flex items-center gap-2 transition-colors ${isDark ? 'text-slate-500 hover:text-indigo-400' : 'text-slate-400 hover:text-indigo-600'}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={toggleTheme} className={`p-2 rounded-xl transition-colors ${isDark ? 'text-slate-400 hover:text-amber-400 hover:bg-slate-800' : 'text-slate-500 hover:text-amber-500 hover:bg-slate-100'}`}>
              {isDark ? <Sun size={18} /> : <Moon size={18} />}
            </button>

            <button onClick={() => setIsChatOpen(!isChatOpen)} className={`p-2 rounded-xl transition-colors flex items-center gap-2 ${isChatOpen ? 'bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400' : (isDark ? 'hover:bg-slate-800 text-slate-400 hover:text-white' : 'hover:bg-slate-100 text-slate-500 hover:text-slate-900')}`}>
        ...
        <div className={`flex-1 overflow-y-auto custom-scrollbar ${isDark ? 'bg-[#0B1121]' : 'bg-transparent'}`}>
          <div className={`w-full border-b ${isDark ? 'bg-black border-slate-800' : 'bg-slate-100 border-slate-200 shadow-sm'}`}>
            <div className="max-w-6xl mx-auto aspect-video relative flex items-center justify-center group overflow-hidden bg-black">
              ...
              {activeItem.type === 'video' ? (
                <div className="w-full h-full bg-black relative flex items-center justify-center shadow-inner">
                  ...
                  {(!videoData || videoData.type === 'none' || videoData.type === 'unknown') && (
                    <div className="flex flex-col items-center justify-center text-slate-500 space-y-4 p-6 text-center">
                      ...
                      <div>
                        <p className={`font-black tracking-widest uppercase text-xs ${isDark ? 'text-white/80' : 'text-white/80'}`}>Video Source Unavailable</p>
                      </div>
                    </div>
                  )}
                </div>
              ) : activeItem.type === 'quiz' ? (
                <div className={`text-center p-8 z-10 w-full h-full flex flex-col items-center justify-center ${isDark ? 'bg-[#151E2E]' : 'bg-gradient-to-br from-indigo-50 via-white to-rose-50'}`}>
                  <div className={`w-24 h-24 rounded-[2rem] flex items-center justify-center mx-auto mb-6 rotate-3 shadow-sm ${isDark ? 'bg-rose-500/10 border-rose-500/20 text-rose-500' : 'bg-white border border-rose-100 text-rose-500 shadow-rose-500/10'}`}>
                    <HelpCircle size={48} className="-rotate-3" />
                  </div>
                  <h2 className={`text-3xl md:text-4xl font-black mb-3 tracking-tight ${isDark ? 'text-white' : 'text-slate-800'}`}>{activeItem.title}</h2>
                  <p className={`font-medium mb-10 max-w-md mx-auto ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>This is a mandatory checkpoint. You must score 80% to unlock the next chapter.</p>
                ...
                <div className={`w-full h-full p-6 md:p-12 overflow-y-auto custom-scrollbar relative z-10 ${isDark ? 'bg-[#151E2E]' : 'bg-white'}`}>
                  <div className="max-w-3xl mx-auto space-y-6">
                    <h1 className={`text-3xl md:text-4xl font-black leading-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>{activeItem.title}</h1>
                    ...
                    <div className={`prose max-w-none ${isDark ? 'prose-invert text-slate-300' : 'text-slate-700'}`}>
                      {activeItem.content ? (
                        ...
                        <div className={`py-20 text-center border-2 border-dashed rounded-3xl ${isDark ? 'border-slate-700 bg-transparent' : 'border-slate-200 bg-slate-50/50'}`}>
                          <FileText size={48} className={`mx-auto mb-4 ${isDark ? 'text-slate-500' : 'text-slate-300'}`} />
                          <p className={`font-black uppercase tracking-widest text-xs ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>No Content Provided</p>
                        </div>
                      )}
                    </div>
          ...
          <div className="max-w-6xl mx-auto p-6 md:p-8 lg:p-10">
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-10">
              <div>
                <h1 className={`text-2xl md:text-3xl font-black mb-2 tracking-tight ${isDark ? 'text-white' : 'text-slate-800'}`}>{activeItem.title}</h1>
                <div className="flex items-center gap-3">
                  <span className={`text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-lg border shadow-sm ${isDark ? 'bg-slate-800 border-slate-700 text-slate-400' : 'bg-white border-slate-200 text-indigo-600'}`}>{activeItem.type}</span>
                  <span className={`text-[11px] font-bold flex items-center gap-1.5 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}><Clock size={14}/> {activeItem.duration || 'N/A'}</span>
                </div>
              </div>
              ...
              <button 
                ...
                className={`shrink-0 w-full md:w-auto px-8 py-4 rounded-2xl font-black text-sm flex items-center justify-center gap-3 transition-all active:scale-95
                  ${isComplete
                    ? (isDark ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/30' : 'bg-emerald-50 text-emerald-600 border border-emerald-200 shadow-sm')
                    : (isDark ? 'bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg shadow-emerald-600/20' : 'bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-white shadow-lg shadow-emerald-500/30')}`}
              >
            ...
            <div className={`flex border-b mb-8 ${isDark ? 'border-slate-800' : 'border-slate-200'}`}>
              ...
              <button onClick={() => setActiveTab('resources')} className={`pb-4 px-6 text-sm font-black uppercase tracking-widest border-b-2 transition-all flex items-center gap-2 ${activeTab === 'resources' ? 'border-indigo-500 text-indigo-500 dark:text-indigo-400' : 'border-transparent text-slate-500 hover:text-slate-800 dark:hover:text-slate-400'}`}>
                Resources <span className={`px-2 py-0.5 rounded text-[10px] ${activeTab === 'resources' ? 'bg-indigo-500/20' : (isDark ? 'bg-slate-800' : 'bg-slate-100')}`}>{activeItem.resources?.length || 0}</span>
              </button>
            </div>

            {activeTab === 'overview' && (
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
                <p className={`font-medium leading-relaxed max-w-3xl text-lg ${isDark ? 'text-slate-300' : 'text-slate-600 whitespace-pre-wrap'}`}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 max-w-5xl animate-in fade-in slide-in-from-bottom-4">
                {activeItem.resources && activeItem.resources.length > 0 ? (
                  activeItem.resources.map(res => (
                    <div key={res.name} className={`p-5 rounded-[1.5rem] border flex items-center justify-between group cursor-pointer transition-all hover:-translate-y-1 shadow-sm ${isDark ? 'bg-[#151E2E] border-slate-800 hover:border-indigo-500/50 hover:shadow-indigo-500/10' : 'bg-white border-slate-200 hover:border-indigo-300 hover:shadow-indigo-500/5'}`}>
                      <div className="flex items-center gap-4 overflow-hidden">
                        <div className={`p-3.5 rounded-2xl shrink-0 transition-colors ${isDark ? 'bg-indigo-500/10 text-indigo-400 group-hover:bg-indigo-600 group-hover:text-white' : 'bg-indigo-50 text-indigo-600 group-hover:bg-indigo-600 group-hover:text-white'}`}><FileText size={20} /></div>
                        <div className="truncate">
                          <h4 className={`font-bold text-sm truncate transition-colors ${isDark ? 'text-slate-200 group-hover:text-white' : 'text-slate-800 group-hover:text-indigo-700'}`}>{res.name}</h4>
                          <p className={`text-[9px] font-black uppercase tracking-widest mt-1 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>{res.size || "Resource"}</p>
                        </div>
                      </div>
                      <button className={`p-2.5 rounded-xl transition-colors ${isDark ? 'text-slate-500 hover:text-indigo-400 hover:bg-slate-800' : 'text-slate-400 hover:text-indigo-600 hover:bg-slate-100'}`}><Download size={18} /></button>
                    </div>
                  ))
                ) : (
                  <p className={`col-span-full py-12 font-black uppercase tracking-widest text-xs text-center ${isDark ? 'text-slate-600' : 'text-slate-400'}`}>No resources attached to this lesson.</p>
                )}
              </div>
            )}
          </div>
  ...
  return (
    <div className={`h-screen w-full flex overflow-hidden font-sans transition-colors duration-500 ${isDark ? 'bg-[#0B1121] text-slate-200' : 'bg-[#FAFAFA] text-slate-800'}`}>
      {renderNavigation()}
      {renderMainContent()}
      {renderChat()}
    </div>
