          ...
          <AnimatePresence>
            {parts.map((part, pIdx) => (
              <motion.div key={part.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className={`rounded-[2rem] md:rounded-[3rem] border overflow-hidden shadow-2xl relative ${isDarkMode ? 'bg-[#151E2E] border-slate-800/80' : 'bg-white border-slate-200'}`}>
                ...
                {/* PART HEADER (TIER 1) */}
                <div className={`backdrop-blur-md p-6 md:p-8 border-b relative z-10 ${isDarkMode ? 'bg-slate-900/90 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
                  <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                    <div className="flex-1 flex flex-col gap-4 w-full">
                       ...
                       <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                          <div className={`flex items-center gap-2 h-10 px-3 rounded-xl border ${isDarkMode ? 'bg-[#0B1121] border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
                          ...
                          <label className={`flex items-center gap-2 h-10 px-3 rounded-xl border cursor-pointer group ${isDarkMode ? 'bg-[#0B1121] border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`} onClick={() => updatePartField(part.id, 'conditionalNavigation', !part.conditionalNavigation)}>
                  ...
                  <AnimatePresence>
                    {part.conditionalNavigation && part.isExpanded && (
                     <div className={`grid grid-cols-1 md:grid-cols-2 gap-4 mt-6 p-5 rounded-2xl border animate-in fade-in ${isDarkMode ? 'bg-[#0B1121]/50 border-slate-800/50' : 'bg-white border-slate-200 shadow-sm'}`}>
                        <div>
                              ...
                              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-8">
                                 ...
                                 <div className="flex items-start gap-3 shrink-0">
                                    <div className="flex flex-col gap-2">
                                       <div className={`flex items-center justify-between gap-3 h-10 px-3 rounded-xl border ${isDarkMode ? 'bg-[#0B1121] border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
                                          <span className="text-[9px] font-black uppercase text-slate-500">Pass %</span>
                                          <input type="number" min="0" max="100" value={section.passPercentage || ''} onChange={(e) => updateSectionField(part.id, section.id, 'passPercentage', e.target.value)} className={`w-12 text-sm font-black outline-none text-right ${isDarkMode ? 'bg-transparent text-indigo-400' : 'bg-transparent text-indigo-600'}`} />
                                       </div>
                                       <label className={`flex items-center justify-between gap-3 h-10 px-3 rounded-xl border cursor-pointer group ${isDarkMode ? 'bg-[#0B1121] border-slate-800' : 'bg-slate-50 border-slate-200'}`} onClick={() => updateSectionField(part.id, section.id, 'conditionalNavigation', !section.conditionalNavigation)}>
                                          <span className={`text-[9px] font-black uppercase tracking-widest ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>Branching</span>
                                          <div className={`relative w-8 h-4 rounded-full transition-colors shrink-0 ${section.conditionalNavigation ? 'bg-amber-500' : 'bg-slate-600'}`}>
                                    ...
                                    <button onClick={() => handleRemoveSection(part.id, section.id)} className={`p-2 h-10 w-10 flex items-center justify-center rounded-xl transition-all active:scale-95 shrink-0 ${isDarkMode ? 'bg-rose-500/10 text-rose-500 hover:bg-rose-500 hover:text-white' : 'bg-rose-50 text-rose-600 hover:bg-rose-500 hover:text-white'}`}><Trash2 size={16}/></button>
                                 </div>
                              </div>
