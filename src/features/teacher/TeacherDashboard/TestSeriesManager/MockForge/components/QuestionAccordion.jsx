...
// Sub-components for the question fields
...
export default function QuestionAccordion({ 
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className={`p-6 md:p-8 rounded-[32px] border relative group ${isDarkMode ? 'bg-[#151E2E] border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
      ...
      {/* Accordion Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 cursor-pointer" onClick={toggleQuestionExpansion}>
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h3 className={`font-black text-lg flex items-center gap-2 ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>
              ...
              <span className={`text-[9px] px-2 py-0.5 rounded-full ${q.difficulty === 'hard' ? 'bg-rose-500/20 text-rose-500' : q.difficulty === 'mid' ? 'bg-amber-500/20 text-amber-500' : 'bg-emerald-500/20 text-emerald-500'}`}>
                {q.difficulty}
              </span>
        ...
        <div className="flex items-center gap-2">
          <button onClick={(e) => { e.stopPropagation(); handleRemoveQuestion(); }} className={`p-2 rounded-xl opacity-0 group-hover:opacity-100 transition-all ${isDarkMode ? 'hover:bg-rose-500/20 text-rose-500' : 'hover:bg-rose-50 text-rose-600'}`}>
            <Trash2 size={18} />
          </button>
            ...
            <div className="pt-8">
              ...
              <div className="space-y-6">
                ...
                {(q.type === 'single_choice' || q.type === 'multiple_choice') ? (
                  <ChoiceEditor q={q} handleAddOption={handleAddOption} handleRemoveOption={handleRemoveOption} updateOptionText={updateOptionText} setCorrectOption={setCorrectOption} updateOptionPoints={updateOptionPoints} isDarkMode={isDarkMode} />
                ) : (
