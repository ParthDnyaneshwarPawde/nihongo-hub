...
import { Hash, Type, BarChart, Layers, Award, TrendingDown, Tag, ListChecks, Timer, AlertCircle } from 'lucide-react';
    ...
    <div className={`p-5 rounded-2xl mb-6 space-y-6 ${isDarkMode ? 'bg-[#0B1121]/80 backdrop-blur-md border border-slate-700/50' : 'bg-slate-50 border border-slate-200'}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
        ...
        <div>
          ...
          {q.gradingRubric && <p className="text-[8px] text-slate-500 mt-1">Auto-calculated sum/max</p>}
        </div>
        <div>
      ...
      <div className={`pt-5 flex flex-col md:flex-row items-start md:items-stretch gap-4 border-t ${isDarkMode ? 'border-slate-800' : 'border-slate-200'}`}>
        
        {/* Toggle 1: Grading Rubric */}
        <div className={`flex items-center justify-between gap-4 w-full md:flex-1 p-3 rounded-xl border transition-colors ${isDarkMode ? 'bg-[#151E2E] border-slate-700/50 hover:border-amber-500/30' : 'bg-white border-slate-200'}`}>
        ...
        {/* Toggle 2: Time Warning with Expanding Note Input */}
        <div className={`flex flex-col gap-3 w-full md:flex-1 p-3 rounded-xl border transition-colors ${isDarkMode ? 'bg-[#151E2E] border-slate-700/50' : 'bg-white border-slate-200'}`}>
          ...
          {/* Expanding Teacher Note Input */}
          {q.afterTimeHint && (
             <div className="relative pt-1 animate-in fade-in slide-in-from-top-2">
                <AlertCircle size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2 mt-0.5 text-rose-500/70" />
                <input 
                   type="text" 
                   value={q.timeWarningMessage || ''} 
                   onChange={(e) => updateQuestionField(q.id, 'timeWarningMessage', e.target.value)}
                   placeholder="e.g. You are running out of time, guess and move on!" 
                   className={`w-full py-1.5 pl-7 pr-3 rounded-lg border text-xs font-bold outline-none transition-all ${isDarkMode ? 'bg-[#0B1121] border-rose-500/30 text-rose-400 placeholder:text-rose-500/40 focus:border-rose-500' : 'bg-rose-50 border-rose-200 text-rose-700 focus:border-rose-400'}`} 
                />
             </div>
          )}
        </div>
