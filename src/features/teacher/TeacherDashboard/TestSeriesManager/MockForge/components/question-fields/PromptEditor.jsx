...
import { Type, Headphones, Clock, Settings2 } from 'lucide-react';
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        {/* NEW: Audio Playback Rule */}
        <div className={`relative flex items-center px-3 rounded-xl border ${isDarkMode ? 'bg-[#0B1121] border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
          <Settings2 size={16} className="text-indigo-500 mr-2" />
          <select value={q.audioPlaybackType || 'R'} onChange={(e) => updateQuestionField(q.id, 'audioPlaybackType', e.target.value)} className={`bg-transparent outline-none font-bold text-xs ${isDarkMode ? 'text-white' : 'text-slate-900'} cursor-pointer`}>
             <option value="P">Pausable (P)</option>
             <option value="R">Replayable (R)</option>
             <option value="O">Strict Play Once (O)</option>
          </select>
        </div>
