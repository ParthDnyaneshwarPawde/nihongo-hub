...
import { AlignLeft, Percent, ShieldAlert, EyeOff, Eye } from 'lucide-react';

export default function GlobalExamSettings({ quizTitle, setQuizTitle, quizDescription, setQuizDescription, passPercentage, setPassPercentage, allowPause, setAllowPause, isLocked, setIsLocked, isDarkMode }) {
  return (
    <section className={`p-8 rounded-[32px] border shadow-sm space-y-8 ${isDarkMode ? 'bg-[#151E2E] border-slate-800' : 'bg-white border-slate-200'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-6 border-t border-dashed border-slate-200 dark:border-slate-800">
        <div className={`p-4 rounded-2xl border flex items-center justify-between ${isDarkMode ? 'bg-[#0B1121] border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Minimum %</p>
            </div>
          </div>
        ...
        <div className={`p-4 rounded-2xl border flex items-center justify-between ${isDarkMode ? 'bg-[#0B1121] border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
          ...
          <button onClick={() => setAllowPause(!allowPause)} className={`relative w-12 h-6 rounded-full transition-colors ${allowPause ? 'bg-amber-500' : 'bg-rose-500'}`}><div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-transform ${allowPause ? 'translate-x-7' : 'left-1'}`}></div></button>
        </div>
        <div className={`p-4 rounded-2xl border flex items-center justify-between ${isDarkMode ? 'bg-[#0B1121] border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">{isLocked ? 'Hidden / Locked' : 'Published'}</p>
            </div>
          </div>
          <button onClick={() => setIsLocked(!isLocked)} className={`relative w-12 h-6 rounded-full transition-colors ${isLocked ? 'bg-slate-600' : 'bg-emerald-500'}`}><div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-transform ${isLocked ? 'left-1' : 'translate-x-7'}`}></div></button>
        </div>
      </div>
    </section>
  );
