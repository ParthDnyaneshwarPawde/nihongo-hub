...
import { 
  ...
  Database, Lock, Unlock, ArrowRight, Sparkles, Layers, Library
} from 'lucide-react';
...
export default function PracticeDojo({ batchId }) {
      ...
      await Promise.all(categories.map(async (cat) => {
        ...
        if (snap.exists()) {
          ...
          locks[cat.id] = {
            ...
            // 🚨 Updated to isLexiconLocked
            isLexiconLocked: data.isLexiconLocked !== false 
          };
        } else {
          locks[cat.id] = { isLocked: false, isLexiconLocked: true };
        }
      }));
  ...
  // 🚨 UPDATED LEXICON TOGGLE
  const toggleLexiconLock = async (catId, e) => {
    e.stopPropagation();
    const currentState = categoryLocks[catId]?.isLexiconLocked ?? true;
    const newState = !currentState;

    setCategoryLocks(prev => ({ ...prev, [catId]: { ...prev[catId], isLexiconLocked: newState } }));

    try {
      const catRef = doc(db, `batches/${batchId}/self_practice/${catId}`);
      await setDoc(catRef, { isLexiconLocked: newState }, { merge: true });
    } catch (error) {
      console.error("Failed to update Lexicon Lock:", error);
    }
  };
      ...
      <div className="relative z-10 space-y-8">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          {categories.map((cat) => {
            ...
            const isLexiconLocked = categoryLocks[cat.id]?.isLexiconLocked ?? true;

            return (
              <motion.div layout key={cat.id} className={`relative group ${isActive ? 'xl:col-span-2' : ''}`}>
                    ...
                    <div className="flex-1 w-full">
                      <div className="flex flex-wrap items-center justify-between gap-4 mb-1">
                        ...
                        <div className="flex items-center gap-2">
                          {!isActive && <div className={`px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-[0.2em] border ${isDarkMode ? 'bg-slate-800/50 border-slate-700 text-slate-400' : 'bg-slate-200/50 border-slate-300 text-slate-500'}`}>{cat.tag}</div>}
                          <button 
                    ...
                    {isActive && (
                      ...
                      >
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 border-t border-slate-800/50 pt-8">
                          ...
                          <button onClick={(e) => { e.stopPropagation(); navigate(`/batch/${batchId}/dojo/${cat.id}/map`); }} className={`flex flex-col items-start p-6 rounded-3xl border-2 transition-all group ${isDarkMode ? 'bg-slate-900/50 border-slate-700 hover:border-indigo-500/50 hover:bg-indigo-500/5' : 'bg-slate-50 border-slate-200 hover:border-indigo-400 hover:bg-indigo-50'}`}>
                            <div className={`p-3 rounded-2xl mb-4 ${isDarkMode ? 'bg-indigo-500/20 text-indigo-400' : 'bg-indigo-100 text-indigo-600'} transition-transform group-hover:scale-110 group-hover:-rotate-3`}><Map size={24} /></div>
                          ...
                          <button onClick={(e) => { e.stopPropagation(); navigate(`/batch/${batchId}/arsenal/${cat.id}`); }} className={`flex flex-col items-start p-6 rounded-3xl border border-dashed transition-all group ${isDarkMode ? 'bg-[#0B1121]/50 border-slate-700 hover:border-slate-500 hover:bg-slate-800' : 'bg-white border-slate-300 hover:border-slate-400 hover:bg-slate-100'}`}>
                            <div className={`p-3 rounded-2xl mb-4 ${isDarkMode ? 'bg-slate-800 text-slate-400' : 'bg-slate-200 text-slate-600'} transition-transform group-hover:scale-110`}><Layers size={24} /></div>
                          ...
                          {/* 🚨 UPDATED LIBRARY BUTTON */}
                          <button 
                            onClick={(e) => { e.stopPropagation(); navigate(`/batch/${batchId}/lexicon/${cat.id}`); }}
                            className={`flex flex-col justify-between p-6 rounded-3xl border transition-all text-left group hover:-translate-y-1 hover:shadow-lg ${
                              isDarkMode ? 'bg-[#0B1121]/50 border-slate-800 hover:border-slate-700' : 'bg-white border-slate-200 hover:border-slate-300'
                            }`}
                          >
                            <div>
                              <div className={`p-3 w-fit rounded-2xl mb-4 transition-transform group-hover:scale-110 ${isDarkMode ? 'bg-slate-800 text-slate-400 group-hover:text-indigo-400' : 'bg-slate-200 text-slate-600 group-hover:text-indigo-600'}`}>
                                <Library size={24} />
                              </div>
                              <h4 className={`text-lg font-black mb-1 ${isDarkMode ? 'text-slate-200' : 'text-slate-800'}`}>Master Library</h4>
                              <p className={`text-xs font-bold mb-6 ${isDarkMode ? 'text-slate-500' : 'text-slate-500'}`}>Manage the global {cat.id} lexicon.</p>
                            </div>

                            {/* Lexicon Lock Toggle (stopPropagation prevents navigation) */}
                            <div 
                              onClick={(e) => toggleLexiconLock(cat.id, e)}
                            ...
                            >
                              {isLexiconLocked ? <><Lock size={16} /> Locked</> : <><Unlock size={16} /> Unlocked</>}
                            </div>
                          </button>
