...
export default function QuestionForge() {
  ...
  const [questions, setQuestions] = useState([
    { 
      ...
      expectedAnswer: '', // 🚨 NEW: Dedicated string for Kanji/Text to prevent array crashes
      solutionText: '',
      solutionVideoUrl: '',
      isDeleted: false
    }
          ...
          if (qIds.length > 0) {
            ...
            const loadedQuestions = questionSnaps
              ...
              .map(snap => {
                ...
                // Safely extract the expected answer for text/kanji types
                let safeExpectedAnswer = '';
                if (qData.type === 'text_input' || qData.type === 'kanji_draw') {
                    safeExpectedAnswer = qData.options && qData.options.length > 0 ? qData.options[0].text : '';
                }
                ...
                return {
                  ...
                  expectedAnswer: safeExpectedAnswer, // Bound to our new dedicated state
  ...
  const handleImportQuestion = async () => {
    ...
    try {
      ...
      if (qSnap.exists()) {
        ...
        let safeExpectedAnswer = '';
        if (data.type === 'text_input' || data.type === 'kanji_draw') {
            safeExpectedAnswer = data.options && data.options.length > 0 ? data.options[0].text : '';
        }
  ...
  const handleRemoveOption = (qId, optId) => {
    setQuestions(questions.map(q => {
      if (q.id === qId) {
        ...
        if (!filteredOptions.some(o => o.isCorrect) && filteredOptions.length > 0) filteredOptions[0].isCorrect = true;
  ...
  const handleSaveQuiz = async () => {
    ...
    // Removed strict prompt validation as requested!
    ...
    try {
      ...
      activeQs.forEach((q) => {
        ...
        // Construct the single correct option for Kanji/Text based on the new dedicated state variable
        if (q.type === 'kanji_draw' || q.type === 'text_input') {
          finalOptions = [{ id: Date.now(), text: q.expectedAnswer, isCorrect: true, count: 0 }];
        }
        ...
        const qData = {
          ...
          options: finalOptions, // 🚨 Saving the cleanly processed options array
          officialSolution: {
            text: q.solutionText,
      ...
      const exercisePayload = {
        ...
        questionIds: lightweightQuestionIds, 
        isCompleted: false
      };
                      ...
                      <div className="pt-8">
                        ...
                        <div className="space-y-6">
                          ...
                          {(q.type === 'single_choice' || q.type === 'multiple_choice') ? (
                            <div>
                              <label className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-2 mb-3 ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}><CheckCircle2 size={14}/> Answer Options</label>
                              <div className="space-y-3 pl-4 border-l-2 border-slate-200 dark:border-slate-800">
                                <AnimatePresence>
                                  {q.options.map((opt, optIndex) => (
                                    <motion.div key={opt.id} initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className={`flex items-center p-3 rounded-2xl border-2 transition-all ${opt.isCorrect ? 'border-emerald-500 bg-emerald-500/10' : isDarkMode ? 'border-slate-700 bg-[#0B1121]' : 'border-slate-200 bg-white'}`}>
                                      <button onClick={() => setCorrectOption(q.id, opt.id)} className={`p-2 shrink-0 transition-colors ${opt.isCorrect ? 'text-emerald-500' : 'text-slate-500'}`}>
                                ...
                                <button onClick={() => handleAddOption(q.id)} className={`w-full py-3 rounded-xl border border-dashed font-bold text-sm mt-2 transition-colors ${isDarkMode ? 'border-slate-700 text-slate-400' : 'border-slate-300 text-slate-500'}`}>+ Add Choice</button>
                              </div>
                            </div>
                          ) : (
                            // 🚨 TEXT INPUT OR KANJI DRAWING (Now isolated using expectedAnswer state)
                              ...
                              <div className="space-y-3 pl-4 border-l-2 border-indigo-200 dark:border-indigo-500/30">
                                ...
                                <div className={`flex items-center p-3 rounded-2xl border-2 transition-all ${isDarkMode ? 'border-indigo-500/50 bg-[#0B1121]' : 'border-indigo-200 bg-white'}`}>
                                  <input 
                                    ...
                                    onChange={(e) => updateQuestionField(q.id, 'expectedAnswer', e.target.value)} 
