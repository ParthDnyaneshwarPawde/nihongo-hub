...
import { 
  ...
  ChevronRight, BarChart3, Sparkles, X, Edit3, Lock, Unlock, 
  ShieldAlert, SlidersHorizontal, Volume2, Play, Pause, RotateCcw, RefreshCcw,
  BookOpen, Bookmark, Image as ImageIcon
} from 'lucide-react';
import { db, auth } from '@services/firebase';
import { collection, getDocs, doc, getDoc, query, setDoc, updateDoc, arrayUnion, arrayRemove, where, limit } from 'firebase/firestore';
...
export default function ListeningLexicon() {
  ...
  const [filterTaskType, setFilterTaskType] = useState('all'); // NEW
  ...
  // 🚨 DICTIONARY & TARGETS STATE (Ported from Reading Lexicon)
  ...
  const taskOptions = [
    { id: 'task_based', label: 'Task-Based' },
    { id: 'point_comp', label: 'Point Comp.' },
  ...
  const loadTargetDictionary = async (entry) => {
    ...
    const processTokens = async (tokens, defaultType) => {
      for (const token of tokens) {
        ...
        } catch (err) { console.error("Dictionary link error:", err); }
      }
    };
  ...
  const handleWordClick = (rawWord) => {
    ...
    const cleanWord = rawWord.split(':')[0].replace(/^[〜~]+/, '').trim();
    if (dictionaryMap[cleanWord]) {
      setLookupTarget({ word: cleanWord, data: dictionaryMap[cleanWord], type: highlightMap[cleanWord] });
    } else {
      setLookupTarget({ word: cleanWord, data: { isRawString: true, text: cleanWord, placeholder: "Definition not mapped." }, type: 'raw' });
    }
  };
  ...
  const filteredData = entries.filter(entry => {
    ...
    const matchTaskType = filterTaskType === 'all' || entry.taskType === filterTaskType; // NEW
    ...
    return matchJlpt && matchTopic && matchTaskType && matchSearch;
  });
  ...
  if (activeListening) {
    if (Array.isArray(activeListening.transcript)) {
      ...
      activeTranscriptLines = [activeListening.transcript];
    } else if (activeListening.transcriptJp || activeListening.transcriptEn || activeListening.transcriptRomaji || activeListening.romaji) {
      activeTranscriptLines = [{ jp: activeListening.transcriptJp || '', en: activeListening.transcriptEn || '', romaji: activeListening.transcriptRomaji || activeListening.romaji || '' }];
    }
  }
                    ...
                    <div>
                      <div className="flex justify-between items-start mb-6 pr-8">
                        <div className="flex gap-2 items-center flex-wrap">
                          ...
                          {doc.taskType && (
                            <span className={`px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border ${isDarkMode ? 'bg-slate-800/80 text-slate-300 border-slate-700' : 'bg-slate-50 text-slate-600 border-slate-200'}`}>
                               {doc.taskType.replace('_', ' ')}
                            </span>
                          )}
                        </div>
          ...
          >
            <header className={`shrink-0 border-b px-4 py-3 flex items-center justify-between shadow-sm z-20 backdrop-blur-2xl ${isDarkMode ? 'bg-[#0F1523]/90 border-slate-800' : 'bg-white/90 border-slate-200'}`}>
              ...
              <div className="flex items-center gap-2">
                {/* 🚨 VOCAB/GRAMMAR TARGETS BUTTON */}
                <button 
                  onClick={() => setShowTargetsDialog(!showTargetsDialog)} 
                  className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all shadow-sm ${showTargetsDialog ? (isDarkMode ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-emerald-100 text-emerald-700 border border-emerald-200') : (isDarkMode ? 'bg-[#151E2E] text-slate-400 border border-slate-700 hover:text-white' : 'bg-white text-slate-500 border border-slate-200 hover:text-slate-900')}`}
                >
                  <BookOpen size={14}/> 
            ...
            {/* 🚨 TARGETS DIALOG (SLIDE DOWN PANEL) */}
            <AnimatePresence>
              {showTargetsDialog && (
                <motion.div 
                  initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
                  className={`border-b overflow-hidden shadow-inner ${isDarkMode ? 'bg-[#0F1523] border-slate-800' : 'bg-slate-50 border-slate-200'}`}
                >
                  <div className="p-6 md:p-8 max-w-5xl mx-auto flex flex-col md:flex-row gap-8">
                    
                    <div className="flex-1">
                      <h4 className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-2 mb-3 ${isDarkMode ? 'text-emerald-400' : 'text-emerald-600'}`}>
                        <Target size={14} /> Target Vocabulary
                      </h4>
                      <div className="flex flex-wrap gap-2">
                        {activeListening.targetVocab ? activeListening.targetVocab.split(',').map((word, i) => {
                          const cleanWord = word.split(':')[0].trim();
                          return (
                            <button key={i} onClick={() => handleWordClick(word)} className={`px-3 py-1.5 rounded-lg text-sm font-bold transition-all border ${isDarkMode ? 'bg-[#151E2E] border-slate-700 hover:border-emerald-500/50 hover:bg-emerald-500/10 text-slate-200' : 'bg-white border-slate-300 hover:border-emerald-400 hover:bg-emerald-50 text-slate-700'}`}>
                              {cleanWord}
                            </button>
                          );
                    ...
                    <div className="flex-1">
                      <h4 className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-2 mb-3 ${isDarkMode ? 'text-fuchsia-400' : 'text-fuchsia-600'}`}>
                        <Sparkles size={14} /> Target Grammar
                      </h4>
                      <div className="flex flex-wrap gap-2">
                        {activeListening.targetGrammar ? activeListening.targetGrammar.split(',').map((word, i) => {
                          const cleanWord = word.split(':')[0].trim();
                          return (
                            <button key={i} onClick={() => handleWordClick(word)} className={`px-3 py-1.5 rounded-lg text-sm font-bold transition-all border ${isDarkMode ? 'bg-[#151E2E] border-slate-700 hover:border-fuchsia-500/50 hover:bg-fuchsia-500/10 text-slate-200' : 'bg-white border-slate-300 hover:border-fuchsia-400 hover:bg-fuchsia-50 text-slate-700'}`}>
                              {cleanWord}
                            </button>
                          );
                        }) : <span className="text-xs text-slate-500">None specified.</span>}
            ...
            <div ref={containerRef} className={`flex-1 overflow-hidden flex relative ${splitOrientation === 'vertical' ? 'flex-row' : 'flex-col'}`}>
              ...
              >
                ...
                {/* 🚨 DICTIONARY LOOKUP MODAL OVERLAY */}
                <AnimatePresence>
                  {lookupTarget && (
                    <motion.div 
                ...
                <div className="max-w-3xl mx-auto pb-24">
                  ...
                  {activeListening.referenceImageUrl && (
                    <div className="mb-12 rounded-[2rem] overflow-hidden border-4 shadow-xl dark:border-slate-800 border-white bg-slate-100 dark:bg-slate-900 flex justify-center">
                      <img 
                        src={activeListening.referenceImageUrl} 
                        alt="Listening Reference Material" 
                  ...
                  <div className="space-y-10">
                    ...
                    {activeTranscriptLines.length > 0 && activeTranscriptLines[0].jp ? (
                        ...
                        {activeTranscriptLines.map((line, idx) => (
                          <div key={idx} className="relative">
                            <div className={`transition-all duration-300 overflow-hidden ${showJp ? 'opacity-100 mb-3' : 'opacity-0 h-0'}`}>
                              <p className={`text-2xl sm:text-3xl font-black leading-relaxed whitespace-pre-wrap ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{line.jp}</p>
                            </div>
                            <div className={`transition-all duration-300 overflow-hidden ${showRomaji ? 'opacity-100 mb-4' : 'opacity-0 h-0'}`}>
                              <p className={`text-base font-bold tracking-wide whitespace-pre-wrap ${isDarkMode ? 'text-amber-400' : 'text-amber-600'}`}>
                                {line.romaji ? line.romaji : <span className="italic opacity-50 text-sm">⚠️ No Romaji provided.</span>}
                              </p>
                            </div>
                            ...
                            <div className={`transition-all duration-300 overflow-hidden ${showEn ? 'opacity-100' : 'opacity-0 h-0'}`}>
                              <p className={`text-lg font-medium leading-relaxed whitespace-pre-wrap ${isDarkMode ? 'text-emerald-300' : 'text-emerald-700'}`}>
                                {line.en ? line.en : <span className="italic opacity-50 text-sm">⚠️ No English Translation provided.</span>}
                              </p>
                            </div>
                        ...
                        return (
                          <div key={qIndex} className={`p-6 sm:p-8 rounded-[2.5rem] border-2 transition-all duration-500 ${showResults ? (isCorrect ? 'border-emerald-500 bg-emerald-500/5 shadow-emerald-500/5' : 'border-rose-500 bg-rose-500/5 shadow-rose-500/5') : (isDarkMode ? 'border-slate-800 bg-[#151E2E] shadow-sm' : 'border-white bg-white shadow-md')}`}>
                            ...
                            <div className="mb-8">
                              ...
                              {q.preQuestion && <p className={`text-sm font-bold mb-4 px-4 py-3 rounded-xl border-l-4 border-indigo-400 bg-indigo-50/50 dark:bg-indigo-500/10 dark:border-indigo-500 ${isDarkMode ? 'text-indigo-200' : 'text-indigo-800'}`}><strong>Pre-Question:</strong> {q.preQuestion}</p>}
                              {q.questionImageUrl && (
                                <div className="mb-6 rounded-2xl overflow-hidden border-2 dark:border-slate-700">
                                  <img src={q.questionImageUrl} alt="Question Context" className="w-full h-auto object-contain max-h-[300px]" />
                                </div>
                              )}
                              <p className={`text-xl sm:text-2xl font-bold leading-snug ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{q.mainQuestion || q.questionJp || "Listen to the audio to answer."}</p>
                              {showEn && q.questionEn && <p className={`text-sm font-medium mt-3 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>{q.questionEn}</p>}
                            </div>
                            ...
                            <div className="space-y-3">
                              {q.options.map((opt, optIndex) => {
                                ...
                                let cardClass = `w-full text-left p-5 rounded-2xl border-2 transition-all duration-300 flex flex-col sm:flex-row items-center gap-4 relative overflow-hidden group `;
                                ...
                                return (
                                  <button key={optIndex} disabled={showResults} onClick={() => handleAnswerSelect(qIndex, optIndex)} className={cardClass}>
                                    <div className="flex items-center gap-4 w-full">
                                      ...
                                      {optImg && <img src={optImg} alt={`Option ${optIndex + 1}`} className="h-16 w-auto object-contain rounded-lg border dark:border-slate-700" />}
                                      {optText && <span className="font-bold text-base break-words break-all flex-1">{optText}</span>}
                                    </div>
                                  </button>
                                );
                              })}
                                  ...
                                  <div className="flex items-start gap-4 p-4 rounded-2xl bg-white dark:bg-[#0B1120] border dark:border-slate-800 shadow-sm">
                                    ...
                                    <div>
                                      ...
                                      <p className={`text-sm font-medium leading-relaxed ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>
                                        {typeof q.explanation === 'object' ? (q.explanation.text || 'No explanation provided.') : (q.explanation || 'No explanation provided.')}
                                      </p>
                                    </div>
                                  </div>
                                </motion.div>
        ...
        {isFilterModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className={`relative w-full max-w-sm rounded-[2rem] border shadow-2xl p-6 md:p-8 ${isDarkMode ? 'bg-[#0F1523] border-slate-800' : 'bg-white border-slate-200'}`}>
              ...
              {/* Task Type Pills */}
              <div className="mb-8">
                <p className={`text-[10px] font-black uppercase tracking-widest mb-3 ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Task Type</p>
                <div className="flex flex-wrap gap-2">
                  <button onClick={() => setFilterTaskType('all')} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all border ${filterTaskType === 'all' ? (isDarkMode ? 'bg-indigo-500/20 border-indigo-500/50 text-indigo-400' : 'bg-indigo-50 border-indigo-200 text-indigo-600') : (isDarkMode ? 'bg-transparent border-slate-800 text-slate-400 hover:border-slate-600' : 'bg-transparent border-slate-200 text-slate-500 hover:border-slate-300')}`}>Any Format</button>
                  {taskOptions.map(opt => (
                    <button key={opt.id} onClick={() => setFilterTaskType(opt.id)} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all border ${filterTaskType === opt.id ? (isDarkMode ? 'bg-indigo-500/20 border-indigo-500/50 text-indigo-400' : 'bg-indigo-50 border-indigo-200 text-indigo-600') : (isDarkMode ? 'bg-transparent border-slate-800 text-slate-400 hover:border-slate-600' : 'bg-transparent border-slate-200 text-slate-500 hover:border-slate-300')}`}>{opt.label}</button>
                  ))}
                </div>
              </div>
