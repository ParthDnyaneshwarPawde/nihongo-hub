...
// VISIBILITY TOGGLES
  const toggleCategoryVisibility = async (e, catId) => {
    ...
    try { 
      await setDoc(doc(db, `batches/${batchId}/lexicon_overrides`, categoryId), { hiddenCategories: newHidden }, { merge: true }); 
    } catch (err) {
  ...
  const toggleGrammarVisibility = async (e, grammarId) => {
    ...
    try { 
      await setDoc(doc(db, `batches/${batchId}/lexicon_overrides`, categoryId), { hiddenGrammar: newHidden }, { merge: true }); 
    } catch (err) {
      console.error("Failed to hide grammar point:", err);
    }
  };
