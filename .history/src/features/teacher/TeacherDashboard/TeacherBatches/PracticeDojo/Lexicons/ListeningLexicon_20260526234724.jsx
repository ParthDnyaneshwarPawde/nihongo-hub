...
import { 
  ...
  GraduationCap, Loader2, ShieldAlert, MessageCircle, Mic, Radio
} from 'lucide-react';
...
export default function ListeningLexicon() {
  ...
  const topicCategories = [
    ...
    { id: 'monologue', label: 'Monologues', jp: '独白', icon: Volume2 },
  ];

  useEffect(() => {
    const debugFirestore = async () => {
      const q = query(collection(db, `lexicons/listening/entries`));
      const snap = await getDocs(q);
      console.log("🔥 Total entries found in 'lexicons/listening/entries':", snap.size);
      snap.forEach(doc => console.log("ID:", doc.id, "Data:", doc.data()));
    };
    debugFirestore();
  }, []);
  ...
  // DYNAMIC FILTERING
  // REPLACE your current displayData with this:
const displayData = entries; // Bypass filter completely
console.log("DEBUG: All entries (bypassing filter):", entries);
