...
import { 
  ...
  Volume2, Image as ImageIcon, AlignLeft, ChevronUp, ChevronDown, Edit2, Search, MessageCircle
} from 'lucide-react';
...
export default function ListeningLexiconEditor() {
  ...
  const downloadTemplate = () => {
    const headers = "ID,TitleJp,TitleEn,JLPT,TaskType, Topic, AudioUrl,RefImageUrl,TargetVocab,TargetGrammar,TranscriptJp, TranscriptRo, TranscriptEn,PreQ,MainQ,QImg,QOpt0Txt,QOpt0Img,QOpt1Txt,QOpt1Img,QOpt2Txt,QOpt2Img,QOpt3Txt,QOpt3Img,QCorrect,QExp\n";
    // Example includes text + image options for A and B, leaving C and D blank for a True/False visual question
    const row1 = `ls_weather,天気予報,Weather Forecast,N4,point_comp,conversation,https://audio.mp3,,天気,〜でしょう,明日は晴れるでしょう。,Ashita wa hareru deshou.,It will be sunny tomorrow.,明日の天気はどうなりますか。,What will the weather be tomorrow?,,晴れ,https://img.com/sun.png,雨,https://img.com/rain.png,,,,0,The audio says sunny.\n`;
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          if (!aggregatedData[trackingKey]) {
            aggregatedData[trackingKey] = {
              ...
              questions: [], topic: entryData.topic || '', transcriptRo: entryData.transcriptro || '',
            };
          }
  ...
  const handleEdit = (entry) => {
    ...
    setFormData({
      ...
      transcriptJp: entry.transcriptJp || '', transcriptEn: entry.transcriptEn || '', questions: parsedQuestions, topic: entry.topic || '', transcriptRo: entry.transcriptRo || '',
    });
          ...
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div className={panelClass}>
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 mb-4">
                ...
                <div>
                  <label className={labelClass}><Radio size={10} className="inline mr-1"/> Task Type</label>
                  <select name="taskType" value={formData.taskType} onChange={handleInputChange} className={inputBaseClass}>
                    {taskTypes.map(t => <option key={t.id} value={t.id}>{t.label}</option>)}
                  </select>
                </div>
                {/* 🚨 NEW TOPIC FIELD */}
                <div>
                  <label className={labelClass}><MessageCircle size={10} className="inline mr-1"/> Topic</label>
                  <select name="topic" value={formData.topic} onChange={handleInputChange} className={inputBaseClass}>
                    <option value="">Select Topic...</option>
                    <option value="conversation">Conversations</option>
                    <option value="announcement">Announcements</option>
                    <option value="interview">Interviews</option>
                    <option value="monologue">Monologues</option>
                  </select>
                </div>
                <div className="col-span-2">
                  <label className={`${labelClass} !text-fuchsia-500`}><Volume2 size={10} className="inline mr-1"/> Main Audio URL (Required)</label>
                  <input type="text" name="audioUrl" value={formData.audioUrl} onChange={handleInputChange} placeholder="https://d1ux...mp3" className={inputBaseClass} required />
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4 border-t border-slate-200 dark:border-slate-800">
                <div className="sm:col-span-1">
                  <label className={labelClass}><ImageIcon size={10} className="inline mr-1"/> Reference Image (Opt)</label>
                  <input type="text" name="referenceImageUrl" value={formData.referenceImageUrl} onChange={handleInputChange} placeholder="Image for 'Look at picture'..." className={inputBaseClass} />
                </div>
                <div>
                  <label className={labelClass}><LinkIcon size={10} className="inline mr-1"/> Target Vocab</label>
                  <input type="text" name="targetVocab" value={formData.targetVocab} onChange={handleInputChange} placeholder="天気, 晴れ" className={inputBaseClass} />
                </div>
                <div>
                  <label className={labelClass}><LinkIcon size={10} className="inline mr-1"/> Target Grammar</label>
                  <input type="text" name="targetGrammar" value={formData.targetGrammar} onChange={handleInputChange} placeholder="〜でしょう" className={inputBaseClass} />
                </div>
              </div>
            </div>
            ...
            <div className={`${panelClass} border-indigo-500/30`}>
              ...
              <div className="space-y-4">
                ...
                {/* 🚨 NEW ROMAJI FIELD */}
                <div>
                  <label className={`text-[9px] font-bold uppercase tracking-wider mb-1 block ${isDarkMode ? 'text-amber-400' : 'text-amber-600'}`}>Romaji Transcript</label>
                  <textarea name="romaji" value={formData.romaji} onChange={handleInputChange} placeholder="onna no hito to otoko no hito ga hanashite imasu..." rows="4" className={`${inputBaseClass} resize-y`} />
                </div>
                <div>
