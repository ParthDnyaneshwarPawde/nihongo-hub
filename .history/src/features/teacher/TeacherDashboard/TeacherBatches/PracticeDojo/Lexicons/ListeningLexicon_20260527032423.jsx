...
import { 
  ...
  ChevronDown, MessageCircle, ListAudio, ArrowLeft, Headphones, ChevronRight
} from 'lucide-react';
...
export default function ListeningLexicon() {
  ...
  const displayData = entries.filter(entry => {
    ...
    const searchTitle = (entry.title || entry.titleJp || entry.titleEn || '').toLowerCase();
  ...
  return (
    <div className={`min-h-screen pb-32 animate-in fade-in duration-700 relative ${isDarkMode ? 'bg-[#0B1120] text-slate-200' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <div className="max-w-[1400px] mx-auto px-6 pt-10 relative z-10 space-y-10">
        
        {/* 🚨 RECREATED READING LIBRARY HEADER */}
        <header className={`flex flex-col xl:flex-row xl:items-center justify-between gap-6 pb-6 border-b ${isDarkMode ? 'border-slate-800/60' : 'border-slate-200'}`}>
          
          <div className="flex items-center gap-5">
             <button 
                onClick={() => navigate(-1)} 
                className={`w-12 h-12 rounded-full flex items-center justify-center transition-all border ${isDarkMode ? 'bg-[#151E2E] border-slate-800 text-slate-400 hover:text-white hover:bg-slate-800' : 'bg-white border-slate-200 text-slate-500 hover:text-slate-900 hover:bg-slate-50'}`}
             >
               <ArrowLeft size={20}/>
             </button>
             <div>
                <div className="flex items-center gap-3">
                  <Headphones size={24} className={isDarkMode ? 'text-indigo-400' : 'text-indigo-600'} />
                  <h2 className={`text-2xl font-black tracking-tight ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                    Listening Library
                  </h2>
                </div>
                <p className={`text-xs font-bold mt-1 ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>
                  Select an audio track to begin your immersion.
                </p>
             </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {/* Search Bar */}
            <div className={`flex items-center gap-3 px-4 py-3 rounded-[2rem] border min-w-[240px] transition-all ${isDarkMode ? 'bg-[#151E2E] border-slate-800 focus-within:border-indigo-500/50' : 'bg-white border-slate-200 focus-within:border-indigo-400/50'}`}>
            ...
            {/* JLPT Filter */}
            <div className={`flex items-center border rounded-[2rem] overflow-hidden shadow-sm transition-all ${isDarkMode ? 'bg-[#151E2E] border-slate-800 hover:border-slate-700' : 'bg-white border-slate-200 hover:border-slate-300'}`}>
              <select value={filterJlpt} onChange={e => setFilterJlpt(e.target.value)} className={`appearance-none bg-transparent pl-5 pr-4 py-3 text-xs font-bold outline-none cursor-pointer w-full ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>
                <option value="all">Level: All</option>
                {jlptOptions.map(opt => <option key={opt} value={opt}>{opt}</option>)}
              </select>
              <div className="pr-4 pointer-events-none text-slate-500"><ChevronDown size={14} /></div>
            </div>

            {/* Topic Filter */}
            <div className={`flex items-center border rounded-[2rem] overflow-hidden shadow-sm transition-all ${isDarkMode ? 'bg-[#151E2E] border-slate-800 hover:border-slate-700' : 'bg-white border-slate-200 hover:border-slate-300'}`}>
              <select value={filterTopic} onChange={e => setFilterTopic(e.target.value)} className={`appearance-none bg-transparent pl-5 pr-4 py-3 text-xs font-bold outline-none cursor-pointer w-full ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          <AnimatePresence>
            {displayData.map((track, index) => {
              ...
              // Fallback getters for database mismatches (Matches Japanese style)
              ...
              return (
                <motion.div 
                  ...
                  className={`group relative p-6 md:p-8 rounded-[2rem] border cursor-pointer transition-all hover:-translate-y-1 shadow-sm flex flex-col justify-between h-full min-h-[280px] ${
                    isDarkMode ? 'bg-[#151E2E]/80 border-slate-800 hover:border-slate-600' : 'bg-white border-slate-200 hover:border-slate-300'
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-8">
                    <div className="flex gap-2">
                      <span className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${isDarkMode ? 'bg-indigo-500/10 text-indigo-400' : 'bg-indigo-50 text-indigo-600'}`}>
                        {track.jlpt || 'N/A'}
                      </span>
                      {track.topic && (
                        <span className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 ${isDarkMode ? 'bg-slate-800/80 text-slate-400' : 'bg-slate-100 text-slate-500'}`}>
                           <ListAudio size={10}/> {track.topic}
                        </span>
                      )}
                    </div>

                    {/* Circular Lock Toggle */}
                    <button 
                      onClick={(e) => toggleAudioVisibility(e, track.id)} 
                      className={`w-9 h-9 rounded-full flex items-center justify-center transition-all border ${
                        isHidden 
                        ? (isDarkMode ? 'bg-rose-500/10 text-rose-500 border-rose-500/20' : 'bg-rose-50 text-rose-600 border-rose-200') 
                        : (isDarkMode ? 'bg-slate-800/50 text-slate-500 border-slate-700 hover:text-emerald-400 hover:bg-emerald-500/10 opacity-0 group-hover:opacity-100' : 'bg-slate-100 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 opacity-0 group-hover:opacity-100')
                      }`}
                    >
                  ...
                  <div className="flex-1 flex flex-col justify-center">
                    <h3 className={`text-2xl md:text-3xl font-black tracking-tight mb-2 line-clamp-2 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                      {trackTitle}
                    </h3>
                    <p className={`text-sm font-medium leading-relaxed line-clamp-2 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                      {trackDesc}
                    </p>
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-8 pt-4">
                    ...
                    {/* Circular Play/Chevron Button */}
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${isDarkMode ? 'bg-slate-800 text-slate-400 group-hover:bg-indigo-500 group-hover:text-white' : 'bg-slate-100 text-slate-500 group-hover:bg-indigo-600 group-hover:text-white'}`}>
...
// ==========================================
function ListeningModal({ entry, onClose, isDarkMode }) {
      ...
      >
        ...
        <div className={`pt-12 pb-8 px-6 sm:px-10 shrink-0 flex flex-col items-center border-b text-center relative overflow-hidden ${isDarkMode ? 'bg-gradient-to-b from-[#1A233A] to-[#0F1523] border-slate-800' : 'bg-gradient-to-b from-indigo-50/50 to-white border-slate-200'}`}>
          ...
          <div className="flex flex-wrap gap-2 justify-center mb-6 relative z-10">
            <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest border ${isDarkMode ? 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20' : 'bg-indigo-50 text-indigo-600 border-indigo-200'}`}>
              {entry.jlpt || 'N/A'}
            </span>
            {entry.topic && (
              <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest border ${isDarkMode ? 'bg-slate-800/80 text-slate-300 border-slate-700' : 'bg-slate-50 text-slate-600 border-slate-200'}`}>
                {entry.topic}
              </span>
            )}
          </div>
