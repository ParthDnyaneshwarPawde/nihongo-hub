...
import { 
  ...
  ChevronUp, ChevronDown, ChevronRight, Edit3, Loader2, Cloud, Play, Plus, Trash2, ShieldAlert
} from 'lucide-react';
...
export default function PracticeMapEditor() {
  ...
  // --- FIREBASE FETCH ENGINE W/ BOUNCER ---
  useEffect(() => {
    let isMounted = true; 

    const checkSecurityAndFetchMap = async () => {
      if (isMounted) setIsLoading(true);
      try {
        const userUid = auth.currentUser?.uid;
        if (!userUid) {
          if (isMounted) { setAccessDenied(true); setIsLoading(false); }
          return;
        }
        ...
        if (lexSnap.exists() && (lexSnap.data().accessIds || []).includes(userUid)) {
          masterAuth = true;
          if (isMounted) setIsMasterTeacher(true);
        }
        ...
        console.error("Error fetching map or checking security:", error); 
        if (isMounted) setAccessDenied(true);
      } finally { 
        if (isMounted) setIsLoading(false); 
      }
    };
    checkSecurityAndFetchMap();
    ...
    return (
      <div className={`min-h-screen flex flex-col items-center justify-center p-6 text-center ${isDarkMode ? 'bg-[#0B1120] text-slate-200' : 'bg-slate-50 text-slate-900'}`}>
        ...
        <p className="text-sm font-medium text-slate-500 max-w-md leading-relaxed">
          You are not a collaborator or lead for this course, so you cannot access the path builder for this batch.
        </p>
