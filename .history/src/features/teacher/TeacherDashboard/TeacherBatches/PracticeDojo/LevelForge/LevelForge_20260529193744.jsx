...
import { 
  ...
  Download, FileSpreadsheet, ChevronDown, ShieldAlert
} from 'lucide-react';
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        // 4. AUTHORIZED: Fetch Existing Node
    ...
    try {
      ...
      if (qSnap.exists()) {
        ...
        setQuestions([...questions, {
          ...
          isExpanded: true, 
          compatibleEngines: finalEngines,
    ...
    return (
      <div className={`min-h-screen flex flex-col items-center justify-center p-6 text-center ${isDarkMode ? 'bg-[#0B1121] text-slate-200' : 'bg-slate-50 text-slate-900'}`}>
        ...
        <p className="text-sm font-medium text-slate-500 max-w-md leading-relaxed">
          You are not a collaborator or lead for this course, so you cannot edit the level forge for this batch.
        </p>
