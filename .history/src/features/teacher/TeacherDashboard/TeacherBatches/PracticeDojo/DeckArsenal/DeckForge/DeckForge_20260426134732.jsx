...
import { 
  ...
  Zap, Target, Lock, Unlock, Globe, ShieldCheck, Coins
} from 'lucide-react';
  ...
  const handleSaveDeck = async () => {
    ...
    try {
      ...
      activeQs.forEach((q) => {
        let finalQId = q.dbId 
            ? q.dbId 
            : (q.customId?.toString().trim() !== '' ? `q_${String(q.customId).padStart(6, '0')}` : `q_${Math.floor(100000 + Math.random() * 900000)}`);
