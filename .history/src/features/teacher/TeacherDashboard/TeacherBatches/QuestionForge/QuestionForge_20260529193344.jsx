...
import { 
  ArrowLeft, Save, Loader2, Sparkles, Upload, Download, 
  FileSpreadsheet, ChevronDown, ShieldAlert 
} from 'lucide-react';
...
export default function QuestionForge() {
  ...
  const [questions, setQuestions] = useState([{ 
    ...
    tags: '', compatibleEngines: ['exam'], prompt: '', mediaUrl: '', timeLimit: 45, points: 4, negativePoints: 1,
    options: [
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        // 1. Check Batch-Specific Authorization
        const batchSnap = await getDoc(doc(db, 'batches', batchId));
        ...
        // 3. AUTHORIZED: Fetch Existing Quiz (Only if editing an existing one)
        if (exerciseId) {
            ...
            if (qIds.length > 0) {
              ...
              const loadedQs = questionSnaps.filter(snap => snap.exists()).map(snap => {
                ...
                return {
                  ...
                  tags: qData.tags ? (Array.isArray(qData.tags) ? qData.tags.join(', ') : qData.tags) : '', prompt: qData.promptText || '', mediaUrl: qData.mediaUrl || '',
                  timeLimit: qData.idealTimeSeconds || 45, points: qData.points || 4, negativePoints: qData.negativePoints || 1, compatibleEngines: qData.compatibleEngines || ['exam'],
              ...
              const purifiedQuestions = data.questions.map(q => ({
                ...q, id: generateUniqueId(), compatibleEngines: q.compatibleEngines || ['exam'], options: q.options?.map(o => ({ ...o, uid: o.uid || generateUniqueId() })) || []
              }));
    ...
    reader.onload = async (event) => {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        parsedQuestions.push({
          ...
          compatibleEngines: rowData.compatibleEngines || ['exam'],
      ...
      if (idsToCheck.length > 0) {
         const checkPromises = idsToCheck.map(async (id) => {
             const formattedId = `q_${String(id).padStart(8, '0')}`;
    ...
    try {
      ...
      if (qSnap.exists()) {
        ...
        setQuestions([...questions, {
          ...
          compatibleEngines: data.compatibleEngines || ['exam'],
          ...
          tags: data.tags ? (Array.isArray(data.tags) ? data.tags.join(', ') : data.tags) : '', prompt: data.promptText || '', mediaUrl: data.mediaUrl || '',
  ...
  const handleAddQuestion = () => setQuestions([...questions, { 
    id: generateUniqueId(), customId: '', compatibleEngines: ['exam'], isExpanded: true, type: 'single_choice', subType: 'reading', difficulty: 'mid', topic: '', subTopic: '', allowSecondAttempt: false, tags: '', prompt: '', mediaUrl: '', timeLimit: 45, points: 4, negativePoints: 1, 
    options: [
      { id: 1, uid: generateUniqueId(), text: '', isCorrect: true, count: 0 }, 
      { id: 2, uid: generateUniqueId(), text: '', isCorrect: false, count: 0 }
    ], 
    ...
    return (
      <div className={`min-h-screen flex flex-col items-center justify-center p-6 text-center ${isDarkMode ? 'bg-[#0B1121] text-slate-200' : 'bg-slate-50 text-slate-900'}`}>
        ...
        <p className="text-sm font-medium text-slate-500 max-w-md leading-relaxed">
          You are not a collaborator or lead for this course, so you cannot edit the question forge for this batch.
        </p>
