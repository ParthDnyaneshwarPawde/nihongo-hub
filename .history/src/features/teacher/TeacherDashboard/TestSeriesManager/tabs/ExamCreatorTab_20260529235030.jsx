...
const openDeploymentModal = (id) => {
  ...
  const commitDeploymentConfiguration = async () => {
    ...
    try {
      ...
      // >>> ADD THIS LINE HERE TO REFRESH LOCAL STATE <<<
      ...
      if (series.tests && Array.isArray(series.tests)) {
        series.tests.forEach(deployment => {
          ...
          if (blueprint) {
            ...
            // 2. Ensure the dates actually exist AND are valid numbers
            const hasStart = deployment.testUnlockDate && !isNaN(start);
            const hasEnd = deployment.testDeadlineDate && !isNaN(end);

            // 3. Strict, cascading status routing
            if (!hasStart && !hasEnd) {
              currentStatus = 'upcoming'; // No dates set -> Staged/Upcoming
            } else if (hasEnd && now > end) {
              currentStatus = 'over'; // Past deadline -> Archived
            } else if (hasStart && now >= start) {
              currentStatus = 'live'; // Past start, before deadline -> Active
            } else {
              currentStatus = 'upcoming'; // Start date is in the future -> Staged/Upcoming
            }
