  ...
  const fetchAllData = async (uid = currentUserId) => {
    ...
    try {
      ...
      const exams = testsSnap.docs.map(d => ({
        ...
        targetLevel: d.data().targetLevel || 'JLPT N4',
        durationMinutes: d.data().durationMinutes || 90, 
        collaborators: d.data().collaborators || [uid],
        authorId: d.data().authorId || uid, // 🚨 FIX: Extract authorId so the UI knows who the Lead is
        visuals: d.data().visuals || { from: '#4F46E5', to: '#7C3AED' }
      })).filter(exam => exam.authorId === uid || (exam.collaborators && exam.collaborators.includes(uid))); 
      // 🚨 FIX: This filter hides exams from teachers who aren't assigned to them!
...
const triggerFormOpen = (test = null) => {
    if (test) {
      ...
      setExamForm({
        ...
        authorId: test.authorId // 🚨 FIX: Now pulling your ownership ID into the form
      });
    } else {
      setEditingId(null);
      setExamForm({ 
        ...
        authorId: currentUserId // 🚨 FIX: Sets you as owner for brand new exams
      });
    }
    setTeacherSearch('');
      ...
      if (series.tests && Array.isArray(series.tests)) {
        series.tests.forEach(deployment => {
          ...
          if (blueprint) {
            ...
            if (!hasStart && !hasEnd) {
              // No dates set at all -> Stays in Staging
              currentStatus = 'upcoming'; 
            } else if (hasStart && !hasEnd) {
              // Only Start Date exists -> Runs forever once started
              const start = new Date(deployment.testUnlockDate).getTime();
              currentStatus = now >= start ? 'live' : 'upcoming';
            } else if (!hasStart && hasEnd) {
              // Only Deadline exists -> Active immediately until deadline
              const end = new Date(deployment.testDeadlineDate).getTime();
              currentStatus = now > end ? 'over' : 'live';
            } else if (hasStart && hasEnd) {
              // Both Dates exist -> Strict Window
              const start = new Date(deployment.testUnlockDate).getTime();
              const end = new Date(deployment.testDeadlineDate).getTime();
              
              if (now > end) {
                currentStatus = 'over'; // Deadline passed
              } else if (now >= start && now <= end) {
                currentStatus = 'live'; // Currently inside the window
              } else {
                currentStatus = 'upcoming'; // Start date is in the future
              }
            }
