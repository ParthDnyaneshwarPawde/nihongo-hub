...
import { 
  ...
  Shield, ArrowRight, BookOpen, Layers,  Shield, ArrowRight, BookOpen, Layers, RotateCw
} from 'lucide-react';
...
export default function ExamCreatorTab() {
  ...
  const fetchAllData = async (uid = currentUserId) => {
    ...
    try {
      ...
      const testsSnap = await getDocs(collection(db, 'mock_tests'));
      const exams = testsSnap.docs.map(d => ({
        id: d.id, ...d.data(), status: d.data().status || 'upcoming', 
  ...
  const commitDeploymentConfiguration = async () => {
    ...
    try {
      ...
      alert("Deployment logic synchronized perfectly.");
      ...
      if (series.tests && Array.isArray(series.tests)) {
        series.tests.forEach(deployment => {
          ...
          if (blueprint) {
            ...
            instances.push({
              ...blueprint,
              id: `${series.id}_${blueprint.id}`, // override ID so it renders uniquely
  ...
  return (
            ...
            <div className={viewMode === 'grid' ? "grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6" : "flex flex-col space-y-1.5"}>
              {runtimeViewChunk.length === 0 ? (
                ...
                runtimeViewChunk.map((test) => {
                  ...
                  if (viewMode === 'list') {
                    return (
                      <div key={test.id} className="flex items-center justify-between p-3.5 border-b border-slate-800/60 hover:bg-slate-800/20 transition-all group">
                         
                         {/* Title & Identity */}
                         <div className="flex items-center gap-4 w-[40%] shrink-0">
                            <div className="w-1 h-8 rounded-full shadow-lg" style={{ background: `linear-gradient(to bottom, ${test.visuals?.from || '#4F46E5'}, ${test.visuals?.to || '#7C3AED'})` }}></div>
                            <div className="overflow-hidden">
                               ...
                               <div className="flex gap-2 items-center mt-1">
                                  ...
                                  <span className="text-[10px] text-slate-500 font-mono tracking-widest">{test.isInstance ? test.originalId : test.id}</span>
                               </div>
                            </div>
                         </div>

                         {/* Highlighted Stats */}
                         <div className="flex items-center justify-end gap-12 flex-1 pr-8 border-r border-slate-800/60">
                            {test.isInstance ? (
                               <>
                                <div className="text-right">
                                   <p className="text-[9px] font-black uppercase text-slate-600 tracking-[0.1em] mb-0.5">Start Time</p>
                                   <p className="text-xs font-black text-indigo-400">{formatDeployDate(test.unlockDate)}</p>
                                </div>
                                <div className="text-right">
                                   <p className="text-[9px] font-black uppercase text-slate-600 tracking-[0.1em] mb-0.5">End Time</p>
                               ...
                               <>
                                <div className="text-right">
                                   <p className="text-[9px] font-black uppercase text-slate-600 tracking-[0.1em] mb-0.5">Target Level</p>
    ...
    <button type="button" onClick={() => fetchAllData(currentUserId)} className="p-2.5 rounded-xl transition-all text-slate-500 hover:text-white" title="Refresh Data">
       <RotateCw size={15} className={isLoading ? "animate-spin text-amber-500" : ""} />
    </button>
    <div className="w-px h-5 bg-slate-800 mx-1"></div>
                            {test.isInstance ? (
                               ...
                               <>
                                 ...
                                 <button onClick={() => alert("Launching Forge Architect...")} className="px-3 py-1.5 rounded-lg bg-amber-500/10 text-amber-500 flex items-center justify-center gap-1 hover:bg-amber-500 hover:text-white transition-all text-[10px] font-black uppercase tracking-widest ml-2" title="Forge Editor"><PenTool size={12}/> Forge</button>
                               </>
                            )}
                         </div>
                      </div>
