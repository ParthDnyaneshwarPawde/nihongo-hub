...
export default function SeriesMatrixPage() {
  ...
  const [expandedSecurityLevel, setExpandedSecurityLevel] = useState(1); // Controls which security level is open
  ...
  const loadMoreExams = async (isInitial = false) => {
    ...
    try {
      ...
      const newExams = querySnapshot.docs.map(d => ({
        ...
        proctoringMode: d.data().proctoringMode || false // 🚨 Fetched directly from mock_exams
      }));
  ...
  const addExamToMatrix = async (test) => {
    ...
    const newMatrixExam = { 
      ...
      proctoringMode: test.proctoringMode || false, // 🚨 Syncs initial state with the Blueprint
      securityManifest: {
  ...
  // UI HELPERS
  // ==========================================
  const inputClass = `w-full px-5 h-14 rounded-2xl bg-[#050810] border border-slate-800 !text-white placeholder-slate-600 font-bold text-sm outline-none focus:border-indigo-500 transition-all shadow-inner [color-scheme:dark]`;
  const labelClass = `block text-[10px] font-black uppercase text-white tracking-widest ml-2 mb-2`;
  ...
  const SecurityToggle = ({ label, description, stateKey, colorClass }) => {
    ...
    return (
      <label className="flex items-center justify-between cursor-pointer group py-2.5">
        <div>
          <span className="text-sm font-black uppercase tracking-widest text-white group-hover:text-slate-300 transition-colors">{label}</span>
          <p className="text-[9px] font-bold text-slate-500 mt-1 max-w-[250px] leading-relaxed">{description}</p>
        </div>
        <div className={`w-12 h-6 rounded-full relative transition-colors shadow-inner shrink-0 ${isChecked ? colorClass : 'bg-[#050810] border border-slate-700'}`}>
    ...
    return (
      <div className="flex flex-col min-h-screen bg-[#050810] w-full p-4 md:p-8 select-none">
        <div className="flex-1 flex flex-col items-center justify-center bg-[#0B1120] border border-slate-800 rounded-[3rem] shadow-[0_20px_60px_rgba(0,0,0,0.5)]">
          ...
          <button onClick={() => navigate(-1)} className="px-10 h-14 bg-slate-900 border border-slate-700 hover:border-indigo-500 hover:bg-indigo-600 text-white rounded-[1.5rem] text-xs font-black uppercase tracking-widest transition-all shadow-lg flex items-center gap-3 active:scale-95">
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col h-full flex-1">
        ...
        <div className="sticky top-0 z-30 p-6 md:px-12 md:py-8 border-b border-slate-800 bg-[#0B1120]/90 backdrop-blur-xl flex flex-col md:flex-row md:items-center justify-between gap-8 shrink-0 shadow-lg">
          <div className="flex items-center gap-6">
            <button onClick={() => navigate(-1)} className="w-14 h-14 rounded-2xl bg-slate-900 border border-slate-700 text-slate-400 flex items-center justify-center hover:bg-slate-800 hover:text-white transition-all shadow-lg shrink-0"><ArrowLeft size={24}/></button>
            <div>
          ...
          <div className="flex items-center gap-4">
            <button onClick={() => setIsFilterDrawerOpen(true)} className="px-6 h-14 rounded-[1.5rem] bg-slate-900 border border-slate-800 hover:border-slate-700 text-slate-300 font-black uppercase tracking-widest text-xs flex items-center gap-3 transition-all shadow-lg active:scale-95 shrink-0"><ListFilter size={18}/> Filters</button>
        ...
        <AnimatePresence>
          {selectedMatrixExam && (
            <div className="fixed inset-0 z-[100] flex justify-end bg-black/40 backdrop-blur-md">
              <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="w-full max-w-3xl h-full bg-[#0B1120] border-l border-slate-800 shadow-[0_0_100px_rgba(0,0,0,0.8)] flex flex-col">
                <div className="p-8 md:p-10 border-b border-slate-800 bg-slate-900/80 flex justify-between items-start shrink-0">
                  <div>
                ...
                <form onSubmit={saveMatrixExamSettings} className="flex-1 overflow-y-auto p-8 md:p-10 custom-scrollbar space-y-12">
                  ...
                  <div className="space-y-6">
                    <h4 className="text-sm font-black uppercase tracking-[0.2em] text-white border-b border-slate-800/80 pb-4 mb-2 flex items-center gap-2"><Power size={18} className="text-rose-500"/> Execution Strategy</h4>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className={`p-4 rounded-2xl border-2 cursor-pointer transition-all ${selectedMatrixExam.executionMode === 'auto' ? 'border-amber-500 bg-amber-500/5' : 'bg-[#050810] border-slate-800 hover:border-slate-700'}`} onClick={() => setSelectedMatrixExam({...selectedMatrixExam, executionMode: 'auto'})}>
                      ...
                      <div className={`p-4 rounded-2xl border-2 cursor-pointer transition-all ${selectedMatrixExam.executionMode === 'hybrid' ? 'border-indigo-500 bg-indigo-500/5' : 'bg-[#050810] border-slate-800 hover:border-slate-700'}`} onClick={() => setSelectedMatrixExam({...selectedMatrixExam, executionMode: 'hybrid'})}>
                      ...
                      <div className={`p-4 rounded-2xl border-2 cursor-pointer transition-all ${selectedMatrixExam.executionMode === 'manual' ? 'border-fuchsia-500 bg-fuchsia-500/5' : 'bg-[#050810] border-slate-800 hover:border-slate-700'}`} onClick={() => setSelectedMatrixExam({...selectedMatrixExam, executionMode: 'manual'})}>
                  ...
                  <div className="mt-12 space-y-6">
                    <div className="flex items-center justify-between border-b border-slate-800 pb-4 mb-6">
                       <div>
                         <h3 className="text-xl font-black text-white flex items-center gap-2"><Shield size={20} className={selectedMatrixExam.proctoringMode ? 'text-indigo-500' : 'text-slate-500'}/> Security Manifest</h3>
                         <p className="text-[10px] font-bold text-slate-500 mt-1">Enable multi-tier proctoring engines.</p>
                       </div>
                       <label className={`w-14 h-7 rounded-full relative transition-colors shadow-inner cursor-pointer ${selectedMatrixExam.proctoringMode ? 'bg-indigo-500' : 'bg-[#050810] border border-slate-700'}`}>
                          <div className={`absolute top-1 w-5 h-5 rounded-full bg-white transition-transform ${selectedMatrixExam.proctoringMode ? 'translate-x-8' : 'left-1'}`}></div>
                          <input type="checkbox" className="hidden" checked={selectedMatrixExam.proctoringMode || false} onChange={e => {
                    ...
                    <AnimatePresence>
                      {selectedMatrixExam.proctoringMode && (
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="space-y-4 overflow-hidden">
                          
                          {/* Level 1 Accordion */}
                          <div className="rounded-[2rem] bg-blue-900/5 border border-blue-900/30 overflow-hidden shadow-inner">
                            <button type="button" onClick={() => setExpandedSecurityLevel(expandedSecurityLevel === 1 ? null : 1)} className="w-full p-6 flex items-center justify-between hover:bg-blue-900/10 transition-colors">
                               <h4 className="text-sm font-black uppercase tracking-[0.2em] text-blue-400 flex items-center gap-2"><Monitor size={18}/> Level 1: Environment Hardening</h4>
                               <ChevronDown className={`text-blue-400 transition-transform ${expandedSecurityLevel === 1 ? 'rotate-180' : ''}`} />
                            </button>
                            <AnimatePresence>
                              {expandedSecurityLevel === 1 && (
                                <motion.div initial={{height:0}} animate={{height:'auto'}} exit={{height:0}} className="overflow-hidden">
                                  <div className="p-6 pt-0 space-y-2 border-t border-blue-900/20 mt-2">
                          ...
                          {/* Level 2 Accordion */}
                          <div className="rounded-[2rem] bg-emerald-900/5 border border-emerald-900/30 overflow-hidden shadow-inner">
                            <button type="button" onClick={() => setExpandedSecurityLevel(expandedSecurityLevel === 2 ? null : 2)} className="w-full p-6 flex items-center justify-between hover:bg-emerald-900/10 transition-colors">
                               <h4 className="text-sm font-black uppercase tracking-[0.2em] text-emerald-400 flex items-center gap-2"><Video size={18}/> Level 2: Active Proctoring</h4>
                               <ChevronDown className={`text-emerald-400 transition-transform ${expandedSecurityLevel === 2 ? 'rotate-180' : ''}`} />
                            </button>
                            <AnimatePresence>
                              {expandedSecurityLevel === 2 && (
                                <motion.div initial={{height:0}} animate={{height:'auto'}} exit={{height:0}} className="overflow-hidden">
                                  <div className="p-6 pt-0 space-y-2 border-t border-emerald-900/20 mt-2">
                                    ...
                                    <div className="pt-6 mt-4 border-t border-emerald-900/30 grid grid-cols-2 gap-6">
                                      <div>
                          ...
                          {/* Level 3 Accordion */}
                          <div className="rounded-[2rem] bg-fuchsia-900/5 border border-fuchsia-900/30 overflow-hidden shadow-inner">
                            <button type="button" onClick={() => setExpandedSecurityLevel(expandedSecurityLevel === 3 ? null : 3)} className="w-full p-6 flex items-center justify-between hover:bg-fuchsia-900/10 transition-colors">
                               <h4 className="text-sm font-black uppercase tracking-[0.2em] text-fuchsia-400 flex items-center gap-2"><Cpu size={18}/> Level 3: AI & Forensics (Enterprise)</h4>
                               <ChevronDown className={`text-fuchsia-400 transition-transform ${expandedSecurityLevel === 3 ? 'rotate-180' : ''}`} />
                            </button>
                            <AnimatePresence>
                              {expandedSecurityLevel === 3 && (
                                <motion.div initial={{height:0}} animate={{height:'auto'}} exit={{height:0}} className="overflow-hidden">
                                  <div className="p-6 pt-0 space-y-2 border-t border-fuchsia-900/20 mt-2">
                  ...
                  <div className="mt-12 space-y-6 border-t border-slate-800 pt-10">
                    <h4 className="text-sm font-black uppercase tracking-[0.2em] text-white border-b border-slate-800/80 pb-4 mb-2 flex items-center gap-2"><Lock size={18} className="text-amber-500"/> Access & Workflow Rules</h4>
