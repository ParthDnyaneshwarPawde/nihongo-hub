...
securityManifest: {
  ...
  const processedMatrixExams = useMemo(() => {
    ...
    let mapped = activeSeries.tests.map(test => {
      ...
      const start = new Date(test.unlockDate).getTime();
      const end = new Date(test.deadlineDate).getTime();
      
      let clockStatus = 'staged';
      const hasStart = test.unlockDate && !isNaN(start);
      const hasEnd = test.deadlineDate && !isNaN(end);
      ...
      let effectiveStatus = test.statusOverride || 'staged';
      if (test.executionMode === 'auto') {
        effectiveStatus = clockStatus;
      } else if (test.executionMode === 'hybrid') {
        if (test.statusOverride !== 'live' && test.statusOverride !== 'archived') {
          effectiveStatus = clockStatus;
        }
      }

      return { ...test, blueprint, effectiveStatus };
    });
    ...
    if (filterState.statuses.length > 0) mapped = mapped.filter(x => filterState.statuses.includes(x.effectiveStatus));
  ...
  const SecurityToggle = ({ label, description, stateKey, colorClass }) => {
    const isChecked = selectedMatrixExam?.securityManifest?.[stateKey] ?? true;
    return (
          ...
          {processedMatrixExams.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {processedMatrixExams.map((matrixExam) => {
                ...
                // 🚨 DECIDE FINAL STATUS BASED ON MODE
                let effectiveStatus = matrixExam.statusOverride || 'staged';
                
                if (matrixExam.executionMode === 'auto') {
                  effectiveStatus = clockStatus; // Auto forces the clock
                } else if (matrixExam.executionMode === 'hybrid') {
                  if (matrixExam.statusOverride !== 'live' && matrixExam.statusOverride !== 'archived') {
                    effectiveStatus = clockStatus; // Hybrid falls back to clock if not manually overridden
                  }
                }
                  ...
                  <div key={matrixExam.testId} className={`p-8 rounded-[2.5rem] border-2 transition-all duration-300 flex flex-col justify-between group shadow-2xl relative overflow-hidden ${cardClasses}`}>
                    ...
                    <div className="relative z-10">
                      ...
                      <div className="flex items-center justify-between p-1.5 rounded-[1rem] bg-slate-50 dark:bg-[#050810] border border-slate-200 dark:border-slate-800/80 mb-6 shadow-inner relative overflow-hidden">
                        ...
                        {/* 🚨 THE FIX: Frosted Glass Overlay blocks clicks in Strict Auto */}
                        {matrixExam.executionMode === 'auto' && (
                          <div className="absolute inset-0 bg-slate-100/60 dark:bg-[#050810]/70 backdrop-blur-[1px] z-10 flex items-center justify-center cursor-not-allowed">
                            <span className="text-[10px] font-black uppercase tracking-widest text-slate-700 dark:text-slate-400 flex items-center gap-1.5">
                              <Lock size={12} className="text-amber-500" /> Auto-Locked
                            </span>
                          </div>
                        )}

                        <button disabled={matrixExam.executionMode === 'auto'} onClick={() => quickToggleExamStatus(matrixExam.testId, 'staged')} className={`flex-1 flex justify-center py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${!isLive && !isArchived ? 'bg-white shadow-sm dark:bg-slate-800 text-slate-800 dark:text-slate-300' : 'text-slate-500 dark:text-slate-600 hover:text-slate-700 dark:hover:text-slate-400'}`}>Staged</button>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-3">
                      {['JLPT N5', 'JLPT N4', 'JLPT N3', 'JLPT N2', 'JLPT N1'].map(s => (
                        <button key={s} onClick={() => toggleFilterArray('levels', s)} className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border ${filterState.levels.includes(s) ? 'bg-fuchsia-500/20 border-fuchsia-500/50 text-fuchsia-400' : isDarkMode ? 'bg-[#050810] border-slate-800 text-slate-500 hover:border-slate-600' : 'bg-white border-slate-200 text-slate-600 hover:border-slate-300'}`}>{s}</button>
                      ))}
                    </div>
                  </div>
          ...
          {selectedMatrixExam && (
            <div className="fixed inset-0 z-[100] flex justify-end bg-black/40 backdrop-blur-md">
              <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className={`w-full max-w-3xl h-full flex flex-col border-l shadow-[0_0_100px_rgba(0,0,0,0.8)] ${isDarkMode ? 'bg-[#0B1120] border-slate-800' : 'bg-white border-slate-200'}`}>
                ...
                <form onSubmit={saveMatrixExamSettings} className="flex-1 overflow-y-auto p-8 md:p-10 custom-scrollbar space-y-12">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div className={`p-4 rounded-2xl border-2 cursor-pointer transition-all ${selectedMatrixExam.executionMode === 'auto' ? 'border-amber-500 bg-amber-50 dark:bg-amber-500/5' : (isDarkMode ? 'bg-[#050810] border-slate-800 hover:border-slate-700' : 'bg-slate-50 border-slate-200 hover:border-slate-300')}`} onClick={() => setSelectedMatrixExam({...selectedMatrixExam, executionMode: 'auto', statusOverride: 'staged'})}>
                      ...
                      <div className={`p-4 rounded-2xl border-2 cursor-pointer transition-all ${selectedMatrixExam.executionMode === 'hybrid' ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-500/5' : (isDarkMode ? 'bg-[#050810] border-slate-800 hover:border-slate-700' : 'bg-slate-50 border-slate-200 hover:border-slate-300')}`} onClick={() => setSelectedMatrixExam({...selectedMatrixExam, executionMode: 'hybrid', statusOverride: 'staged'})}>
                      ...
                      <div className={`p-4 rounded-2xl border-2 cursor-pointer transition-all ${selectedMatrixExam.executionMode === 'manual' ? 'border-fuchsia-500 bg-fuchsia-50 dark:bg-fuchsia-500/5' : (isDarkMode ? 'bg-[#050810] border-slate-800 hover:border-slate-700' : 'bg-slate-50 border-slate-200 hover:border-slate-300')}`} onClick={() => setSelectedMatrixExam({...selectedMatrixExam, executionMode: 'manual'})}>
                  ...
                  <div className={`mt-12 space-y-6 border-t pt-10 ${isDarkMode ? 'border-slate-800' : 'border-slate-200'}`}>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      ...
                      <div className="flex flex-col justify-center">
                        <label className="flex items-center justify-between cursor-pointer group mt-4">
                          <div>
                            <span className={`text-sm font-black uppercase tracking-widest group-hover:text-amber-400 transition-colors ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Dynamic Entry Lockout</span>
                            <p className={`text-[9px] font-bold mt-1 max-w-[200px] leading-relaxed ${isDarkMode ? 'text-slate-500' : 'text-slate-500'}`}>Blocks late arrivals after launch.</p>
                          </div>
                          <div className={`w-12 h-6 rounded-full relative transition-colors shadow-inner ${selectedMatrixExam.isEntryLockoutEnabled ? 'bg-amber-500' : (isDarkMode ? 'bg-[#050810] border border-slate-700' : 'bg-slate-200 border border-slate-300')}`}>
                    ...
                    {selectedMatrixExam.isEntryLockoutEnabled && (
                      <div className={`flex items-center gap-4 animate-in fade-in p-4 rounded-[1.5rem] border ${isDarkMode ? 'bg-[#050810] border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
                        ...
                        <span className={`text-[10px] font-black uppercase tracking-widest ${isDarkMode ? 'text-slate-500' : 'text-slate-500'}`}>Minutes allowed for late entry</span>
                      </div>
                    )}

                    <div className={`pt-6 mt-6 border-t ${isDarkMode ? 'border-slate-800/60' : 'border-slate-200'}`}>
                      <label className="flex items-center justify-between cursor-pointer group">
                        <div>
                          <span className={`text-sm font-black uppercase tracking-widest group-hover:text-indigo-400 transition-colors ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Asynchronous Post-Deadline Allocations</span>
                          <p className={`text-[9px] font-bold mt-1 max-w-[300px] leading-relaxed ${isDarkMode ? 'text-slate-500' : 'text-slate-500'}`}>Permit self-paced review workflows after main synchronized sequence ends.</p>
                        </div>
                        <div className={`w-12 h-6 rounded-full relative transition-colors shadow-inner ${selectedMatrixExam.allowPostDeadline ? 'bg-indigo-500' : (isDarkMode ? 'bg-[#050810] border border-slate-700' : 'bg-slate-200 border border-slate-300')}`}>
                ...
                <div className={`p-8 border-t shrink-0 ${isDarkMode ? 'border-slate-800 bg-[#0B1120] shadow-[0_-10px_30px_rgba(0,0,0,0.5)]' : 'border-slate-200 bg-white shadow-[0_-10px_30px_rgba(0,0,0,0.05)]'}`}>
  <button onClick={saveMatrixExamSettings} disabled={isSaving} className={`w-full h-16 rounded-[2rem] text-xs font-black uppercase tracking-[0.2em] flex items-center justify-center gap-3 hover:scale-[1.02] active:scale-95 transition-all shadow-[0_10px_40px_rgba(0,0,0,0.15)] ${isDarkMode ? 'bg-white text-black' : 'bg-slate-900 text-white'}`}>
