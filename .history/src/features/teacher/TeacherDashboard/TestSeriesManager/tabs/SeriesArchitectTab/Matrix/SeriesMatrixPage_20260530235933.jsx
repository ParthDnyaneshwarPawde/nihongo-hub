...
securityManifest: {
          ...
          {processedMatrixExams.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {processedMatrixExams.map((matrixExam) => {
                ...
                // 🚨 NEW: Real-Time System Clock Calculation
  ...
  if (matrixExam.unlockDate && matrixExam.deadlineDate) {
    if (now > end) clockStatus = 'archived';
    else if (now >= start && now <= end) clockStatus = 'live';
  }

  // 🚨 Decide final status based on the mode
  let effectiveStatus = matrixExam.statusOverride; // Default to manual override
  
  if (matrixExam.executionMode === 'auto') {
    effectiveStatus = clockStatus; // Auto forces the clock
  } else if (matrixExam.executionMode === 'hybrid' && matrixExam.statusOverride === 'staged') {
    effectiveStatus = clockStatus; // Hybrid uses clock UNLESS manually forced live/archived
  }
