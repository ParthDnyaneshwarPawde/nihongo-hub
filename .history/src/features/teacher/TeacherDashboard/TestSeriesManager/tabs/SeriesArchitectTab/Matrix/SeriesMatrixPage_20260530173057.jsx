      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col h-full flex-1">
        ...
        <div className="sticky top-0 z-30 p-6 md:px-12 md:py-8 border-b border-slate-800 bg-[#0B1120]/90 backdrop-blur-xl flex flex-col md:flex-row md:items-center justify-between gap-8 shrink-0 shadow-lg">
          <div className="flex items-center gap-6">
            <button onClick={() => navigate(-1)} className="w-14 h-14 rounded-2xl bg-slate-900 border border-slate-700 text-slate-400 flex items-center justify-center hover:bg-slate-800 hover:text-white transition-all shadow-lg shrink-0">
              <ArrowLeft size={24}/>
            </button>
            <div>
                    ...
                    <div className="relative z-10">
                      ...
                      <div className="flex flex-col gap-2 mt-4 text-[9px] font-bold uppercase tracking-widest text-slate-500">
                        {['auto', 'hybrid'].includes(matrixExam.executionMode) && (
                          <>
                            <div className="flex justify-between items-center bg-slate-900/50 p-3 rounded-xl border border-slate-800/80"><span className="text-emerald-500 flex items-center gap-1.5"><Play size={12}/> Unlock:</span> <span className="text-slate-300">{formatDeployDate(matrixExam.unlockDate)}</span></div>
                            <div className="flex justify-between items-center bg-slate-900/50 p-3 rounded-xl border border-slate-800/80"><span className="text-rose-500 flex items-center gap-1.5"><StopCircle size={12}/> Deadline:</span> <span className="text-slate-300">{formatDeployDate(matrixExam.deadlineDate)}</span></div>
                          </>
                        )}
            ...
            <div className="fixed inset-0 z-[100] flex justify-end bg-black/40 backdrop-blur-md">
              <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="w-full max-w-3xl h-full bg-[#0B1120] border-l border-slate-800 shadow-[0_0_100px_rgba(0,0,0,0.8)] flex flex-col">
                ...
                <form onSubmit={saveMatrixExamSettings} className="flex-1 overflow-y-auto p-8 md:p-10 custom-scrollbar space-y-12">
                  ...
                  {/* 🚨 THE NEW PROCTORING & SECURITY MANIFEST */}
                  <div className="mt-12 space-y-10">
                    <h3 className="text-2xl font-black text-white mb-6 border-b border-slate-800 pb-4">Security Manifest</h3>

                    {/* Level 1: Environment Hardening */}
                    <div className="p-8 rounded-[2.5rem] bg-blue-900/5 border border-blue-900/30 space-y-4 shadow-inner">
                      <h4 className="text-sm font-black uppercase tracking-[0.2em] text-blue-400 mb-6 flex items-center gap-2"><Monitor size={18}/> Level 1: Environment Hardening</h4>
                    ...
                    {/* Level 2: Active Proctoring */}
                    <div className="p-8 rounded-[2.5rem] bg-emerald-900/5 border border-emerald-900/30 space-y-6 shadow-inner">
                      <h4 className="text-sm font-black uppercase tracking-[0.2em] text-emerald-400 mb-6 flex items-center gap-2"><Video size={18}/> Level 2: Active Proctoring</h4>
                      ...
                      <div className="pt-6 border-t border-emerald-900/30 grid grid-cols-2 gap-6">
                        <div>
                    ...
                    {/* Level 3: AI Analytics */}
                    <div className="p-8 rounded-[2.5rem] bg-fuchsia-900/5 border border-fuchsia-900/30 space-y-4 shadow-inner">
                      <h4 className="text-sm font-black uppercase tracking-[0.2em] text-fuchsia-400 mb-6 flex items-center gap-2"><Cpu size={18}/> Level 3: AI & Forensics (Enterprise)</h4>
                ...
                <div className="p-8 border-t border-slate-800 bg-[#0B1120] shrink-0 shadow-[0_-10px_30px_rgba(0,0,0,0.5)] flex gap-4">
                  <button type="button" onClick={() => setSelectedMatrixExam(null)} className="px-8 font-black uppercase tracking-widest text-xs text-slate-400 hover:text-white transition-all rounded-[2rem] border border-slate-700 hover:bg-slate-800">
                    Discard 
                  </button>
                  <button onClick={saveMatrixExamSettings} disabled={isSaving} className="flex-1 h-16 bg-gradient-to-r from-orange-500 to-amber-500 text-white rounded-[2rem] text-xs font-black uppercase tracking-[0.2em] flex items-center justify-center gap-3 hover:scale-[1.02] active:scale-95 transition-all shadow-[0_10px_40px_rgba(245,158,11,0.2)]">
