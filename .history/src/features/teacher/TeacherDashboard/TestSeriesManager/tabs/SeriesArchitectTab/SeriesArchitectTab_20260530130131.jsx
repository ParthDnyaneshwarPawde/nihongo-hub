...
export default function SeriesArchitectTab() {
  ...
  // ==========================================
  // STAFF & BATCH MUTATION MECHANICS (INSTANT SYNC)
  // ==========================================
  const toggleCollab = async (id) => {
    const currentCollabs = formData.collaborators || [];
    const newCollabs = currentCollabs.includes(id) ? currentCollabs.filter(x => x !== id) : [...currentCollabs, id];
    ...
    if (editingId) {
      ...
      } catch (err) { console.error("Failed to sync collab to database:", err); }
    }
  };

  const toggleBatch = async (id) => {
    const currentBatches = formData.assignedBatches || [];
    const newBatches = currentBatches.includes(id) ? currentBatches.filter(x => x !== id) : [...currentBatches, id];
    ...
    if (editingId) {
      ...
      } catch (err) { console.error("Failed to sync batch to database:", err); }
    }
  };

  // Search Logic for Collabs (With Safety Fallback)
