...
import { 
  ...
  Clock, Lock, Key, Settings, Server, ToggleLeft, Globe, Repeat, ShieldAlert
} from 'lucide-react';
...
export default function SeriesMatrixPage() {
  ...
  // 🚨 THE NEW ACCESS RESTRICTED SCREEN
  if (!isAuthorized) {
    return (
      <div className="flex flex-col min-h-screen bg-[#050810] w-full p-4 md:p-8 select-none">
        <div className="flex-1 flex flex-col items-center justify-center bg-[#0B1120] border border-slate-800 rounded-[3rem] shadow-[0_20px_60px_rgba(0,0,0,0.5)]">
          ...
          <p className="text-sm font-bold text-slate-400 max-w-md text-center leading-relaxed mb-10">
            You are not listed as the Lead Architect or an authorized Collaborator for this Execution Matrix.
          </p>
          <button 
            onClick={() => navigate('/test-series')} 
            className="px-10 h-14 bg-slate-900 border border-slate-700 hover:border-indigo-500 hover:bg-indigo-600 text-white rounded-[1.5rem] text-xs font-black uppercase tracking-widest transition-all shadow-lg flex items-center gap-3 active:scale-95"
          >
