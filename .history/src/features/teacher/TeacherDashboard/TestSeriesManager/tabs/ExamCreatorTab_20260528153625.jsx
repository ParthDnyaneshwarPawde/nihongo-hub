...
import { 
  ...
  Send, Lock, Filter, LayoutGrid, LayoutList, 
...
export default function ExamCreatorTab() {
  ...
  useEffect(() => {
    const syncDatabase = async () => {
      try {
        ...
        // 1. Fetch Blueprints (No schedule or live status tracking needed here)
  ...
  const commitFormToDatabase = async (e) => {
    e.preventDefault();
    if (!examForm.title.trim()) return alert("Exam instance title is required.");
      ...
      <AnimatePresence mode="wait">
        {isExamFormOpen ? (
          ...
          <motion.form 
            ...
            onSubmit={commitFormToDatabase} className="w-full space-y-10 pb-32"
          >
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 animate-in slide-in-from-right-8 duration-500">
              ...
              <div className="space-y-8 flex flex-col">
                ...
                {/* StepOne Collaborator UI Integration */}
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-slate-950/50 p-8 rounded-[3rem] border border-slate-800 shadow-inner flex flex-col space-y-6">
                    ...
                    {examForm.entryLockout && (
                      <div className="flex items-center gap-4 p-3 rounded-2xl bg-[#0B1120] border border-slate-800 animate-fadeIn">
                        <span className="text-xs font-black uppercase tracking-widest text-slate-500 pl-4">Lock Room After:</span>
                        <input type="number" name="lockoutMinutes" value={examForm.lockoutMinutes} onChange={handleFormInput} className="w-20 p-3 rounded-xl bg-[#0D1527] border border-slate-700 text-white text-center font-black outline-none focus:border-rose-500" min="1" />
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest pr-4">Mins</span>
                      </div>
                    )}
                    ...
                    {examForm.allowLateAttempts && (
                      <div className="flex flex-col gap-2 p-4 rounded-2xl bg-[#0B1120] border border-slate-800 animate-fadeIn mt-2">
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 pl-2">Absolute Final Cutoff Date:</span>
                        <input type="datetime-local" name="lateDeadline" value={examForm.lateDeadline} onChange={handleFormInput} className="flex-1 p-3 rounded-xl bg-[#0D1527] border border-slate-700 text-white text-xs font-bold outline-none focus:border-emerald-500" />
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="flex gap-4 sticky bottom-6 z-30 bg-[#090E1A]/95 backdrop-blur-md p-3 rounded-2xl border border-slate-800 shadow-2xl max-w-lg mx-auto mt-10">
              <button type="button" onClick={closeForm} className="px-6 py-3 rounded-xl text-xs font-bold text-slate-400 hover:bg-slate-800 hover:text-white transition-all uppercase tracking-wider">Discard</button>
          ...
          /* ====================================================================
             WORKSPACE SUB-VIEW: EXAM BLUEPRINT DIRECTORY
             ==================================================================== */
