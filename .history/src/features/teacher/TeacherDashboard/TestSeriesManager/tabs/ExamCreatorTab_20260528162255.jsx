...
import { 
  ...
  Radio, Play, Square, AlertTriangle, Globe, Sparkles, Trash2, Loader2,
  Shield, ArrowRight
} from 'lucide-react';
...
export default function ExamCreatorTab() {
  ...
  const queryTeachersFiltered = useMemo(() => {
    return teacherList.filter(t => 
      ...
      (t.name.toLowerCase().includes(collabSearch.toLowerCase()) || 
       (t.email && t.email.toLowerCase().includes(collabSearch.toLowerCase())))
    );
  }, [teacherList, collabSearch, examForm.collaborators, currentUserId]);
