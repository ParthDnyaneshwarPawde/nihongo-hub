                ...
                runtimeViewChunk.map((test) => {
                  ...
                  if (test.status === 'upcoming') {
                    ...
                    executionActionBtn = (
                      <button 
                        type="button" onClick={() => advanceExamState(test.id, test.status)}
                        className="px-3 py-2 bg-emerald-500/10 hover:bg-emerald-600 text-emerald-400 hover:text-white border border-emerald-500/30 text-[9px] font-black uppercase tracking-widest rounded-xl transition-all duration-200 flex items-center gap-1.5 shadow-sm shadow-emerald-500/5"
                        title="Force start full live countdown manually for all connected users."
                      >
                      ...
                      <div key={test.id} className="pl-3 pr-4 py-2 rounded-xl bg-[#0B1120]/90 border border-slate-800/50 hover:border-slate-700/80 transition-all duration-200 flex flex-col md:flex-row items-center justify-between gap-4 group">
                        ...
                        <div className="flex items-center gap-4 w-full md:w-auto shrink-0 justify-end">
                          ...
                          <div className="flex items-center gap-0.5 bg-[#0D1527]/80 p-1 rounded-xl border border-slate-800/80">
                            ...
                            <button onClick={() => alert("Redirecting to Forge...")} className="p-2 rounded-lg bg-amber-500/10 hover:bg-amber-500 text-amber-500 hover:text-white transition-colors" title="Launch Design Forge"><PenTool size={13}/></button>
                          </div>
                        </div>
                      </div>
                    );
                    ...
                    <div key={test.id} className="p-6 rounded-[2rem] bg-[#0B1120] border border-slate-800/80 hover:border-slate-700 transition-all duration-300 shadow-xl flex flex-col justify-between group relative overflow-hidden">
                      ...
                      <div className="space-y-3 pt-4 border-t border-slate-800/60">
                        ...
                        <div className="grid grid-cols-4 gap-2">
                          ...
                          <button onClick={() => alert("Redirecting to Forge...")} className="py-2.5 bg-gradient-to-r from-amber-600 to-orange-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl flex items-center justify-center gap-1.5 shadow-md shadow-amber-600/5"><PenTool size={12}/> Forge</button>
                        </div>
                      </div>
                    </div>
                  );
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-md bg-black/60">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-3xl max-h-[85vh] flex flex-col rounded-[2.5rem] bg-[#0B1120] border border-slate-800 shadow-2xl overflow-hidden z-10">
              ...
              <div className="p-6 border-t border-slate-800 bg-[#0B1120] flex justify-end gap-4">
                ...
                <button type="button" onClick={commitDeploymentConfiguration} disabled={seriesAssignForm.selectedSeries.length === 0 || isSaving} className="px-8 py-3.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-black uppercase tracking-widest text-[10px] shadow-lg shadow-indigo-600/10 disabled:opacity-30 disabled:cursor-not-allowed transition-all flex items-center gap-2">
                  {isSaving ? <Loader2 size={12} className="animate-spin" /> : <Send size={12} />} Execute Pipeline Allocation
                </button>
              </div>
            </motion.div>
          </div>
