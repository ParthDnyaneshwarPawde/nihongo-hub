...
export default function ExamCreatorTab() {
  ...
  // UI STYLING CLASSES 
  // ==========================================
  const inputBase = `w-full px-6 h-14 rounded-2xl bg-slate-900/80 backdrop-blur-sm border border-slate-800/80 text-white font-bold text-sm transition-all outline-none focus:border-indigo-500 focus:bg-slate-900 hover:border-slate-700`;
  const labelPremiumClass = `block text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] ml-2 mb-2.5 flex items-center gap-2`;
  ...
  const fetchAllData = async (uid = currentUserId) => {
    ...
    try {
      ...
      const exams = testsSnap.docs.map(d => ({
        id: d.id, ...d.data(), status: d.data().status || 'blueprint', 
        description: d.data().description || '', targetLevel: d.data().targetLevel || 'JLPT N4',
        durationMinutes: d.data().durationMinutes || 90, collaborators: d.data().collaborators || [uid],
      ...
      try {
        ...
        if (!staffSnap.empty) {
          setTeacherList(staffSnap.docs.map(d => ({
            id: d.id, name: d.data().displayName || d.data().name || 'Unknown Sensei', email: d.data().email || '',
            role: d.data().role || 'Instructor', avatar: (d.data().displayName || d.data().name || 'ST').substring(0, 2).toUpperCase()
          })));
        }
      } catch (e) {
  ...
  const commitDeploymentConfiguration = async () => {
    ...
    try {
      ...
      // 🚨 FIX: Re-fetch the data so the UI updates instantly into the Upcoming Tab!
      await fetchAllData(currentUserId);
      setActiveFilter('upcoming'); // Auto-switch to upcoming to show them their success
      ...
      if (series.tests && Array.isArray(series.tests)) {
        series.tests.forEach(deployment => {
          ...
          if (blueprint) {
            ...
            // 🚨 FIX: Bulletproof Status Calculus
            if (hasEnd && now > end) {
              currentStatus = 'over'; // Passed deadline
            } else if (hasStart && now >= start) {
              currentStatus = 'live'; // Past start, before deadline
            } else {
              currentStatus = 'upcoming'; // No dates set, or future start date
            }
      ...
      <AnimatePresence mode="wait">
        {isExamFormOpen ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 animate-in slide-in-from-right-8 duration-500">
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-3">
                  ...
                  <textarea name="description" value={examForm.description} onChange={handleFormInput} className={`${inputBase} min-h-[120px] py-4 resize-y`} placeholder="Briefly describe the contents of this exam..." />
                </div>
                ...
                {/* Visual Colors */}
                <div className="p-8 rounded-[2rem] bg-indigo-900/10 border border-indigo-500/20 space-y-6 shadow-inner relative overflow-hidden">
                  ...
                  <div className="grid grid-cols-2 gap-6 relative z-10">
                    <div>
                      ...
                      <div className="p-1 rounded-[1.5rem] bg-slate-900 border border-slate-700 hover:border-indigo-500 transition-all">
                  ...
                  <div className="flex-1 bg-slate-900/40 backdrop-blur-xl p-8 md:p-10 rounded-[3rem] border border-white/5 shadow-2xl flex flex-col relative overflow-hidden">
                    ...
                    <div className="flex gap-3 relative z-10 mb-6 shrink-0">
                      <input 
                        ...
                        className="flex-1 h-14 bg-slate-950/80 border border-slate-800 rounded-2xl px-6 text-sm text-white font-bold outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all shadow-inner" 
                        placeholder="Search Sensei Name or Email..." 
                      />
                      <button type="button" className="bg-gradient-to-br from-indigo-500 to-indigo-600 h-14 w-14 flex items-center justify-center rounded-2xl text-white shadow-[0_5px_20px_rgba(99,102,241,0.3)] hover:scale-105 transition-all">
                        <Search size={20}/>
                      </button>
                    </div>
          ...
          <motion.div key="grid" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
           ...
           {/* Filter Bar */}
            <div className="flex flex-col lg:flex-row lg:items-center justify-between border border-slate-800 rounded-2xl p-2 mb-6 bg-[#0B1120]/40 gap-4">
               <div className="flex flex-wrap items-center gap-2">
                  <button onClick={() => setActiveFilter('all')} className={`flex items-center gap-2 px-5 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${activeFilter === 'all' ? 'bg-indigo-600/20 border border-indigo-500/30 text-indigo-400' : 'text-slate-500 hover:text-white border border-transparent hover:bg-slate-800/50'}`}>
                     <Filter size={14}/> Core Blueprints
                  </button>
                  <button onClick={() => setActiveFilter('active')} className={`flex items-center gap-2 px-5 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${activeFilter === 'active' ? 'bg-emerald-500/20 border border-emerald-500/30 text-emerald-400' : 'text-slate-500 hover:text-white border border-transparent hover:bg-slate-800/50'}`}>
                     <Activity size={14}/> Active Live
                  </button>
                  <button onClick={() => setActiveFilter('upcoming')} className={`flex items-center gap-2 px-5 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${activeFilter === 'upcoming' ? 'bg-amber-500/20 border border-amber-500/30 text-amber-400' : 'text-slate-500 hover:text-white border border-transparent hover:bg-slate-800/50'}`}>
                     <Calendar size={14}/> Staged / Upcoming
                  </button>
                  <button onClick={() => setActiveFilter('over')} className={`flex items-center gap-2 px-5 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${activeFilter === 'over' ? 'bg-slate-800 text-white border border-slate-700' : 'text-slate-500 hover:text-white border border-transparent hover:bg-slate-800/50'}`}>
                     <CheckCircle2 size={14}/> Archived Pipeline
                  </button>
               </div>
               <div className="flex items-center justify-end gap-1 pr-2">
                  <button onClick={() => fetchAllData(currentUserId)} className="p-2.5 rounded-xl transition-all text-slate-500 hover:text-white hover:bg-slate-800/50" title="Force Sync">
                     <RotateCw size={16} className={isLoading ? "animate-spin text-amber-500" : ""} />
                  </button>
                  
                  <div className="w-px h-5 bg-slate-800 mx-2"></div>

                  <button onClick={() => setViewMode('grid')} className={`p-2.5 rounded-xl transition-all ${viewMode === 'grid' ? 'bg-slate-800/80 text-amber-500' : 'text-slate-500 hover:text-white hover:bg-slate-800/50'}`}><LayoutGrid size={16}/></button>
                  <button onClick={() => setViewMode('list')} className={`p-2.5 rounded-xl transition-all ${viewMode === 'list' ? 'bg-slate-800/80 text-amber-500' : 'text-slate-500 hover:text-white hover:bg-slate-800/50'}`}><LayoutList size={16}/></button>
               </div>
            </div>

            {/* Render Window */}
            <div className={viewMode === 'grid' ? "grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6" : "flex flex-col space-y-2"}>
              {runtimeViewChunk.length === 0 ? (
                <div className="py-24 border-2 border-dashed border-slate-800/60 rounded-[3rem] bg-[#0B1120]/40 flex flex-col items-center text-center px-4 col-span-full">
                  ...
                  <p className="text-sm font-black text-white uppercase tracking-widest mb-1">No Assessment Data Found</p>
                  <p className="text-xs text-slate-500 font-bold max-w-sm leading-relaxed">Adjust your filters above, or create your first core exam structure to begin deploying.</p>
                </div>
              ) : (
                runtimeViewChunk.map((test) => {
                  ...
                  if (viewMode === 'list') {
                  return (
                    <div key={test.id} className="flex flex-col md:flex-row md:items-center justify-between p-4 rounded-2xl border border-slate-800/60 bg-[#0B1120]/40 hover:bg-slate-800/40 hover:border-slate-700 transition-all group overflow-hidden gap-4">
                       
                       {/* Title & Identity */}
                       <div className="flex items-center gap-4 w-full md:w-[35%] shrink-0">
                          <div className="w-1.5 h-10 rounded-full shadow-lg shrink-0" style={{ background: `linear-gradient(to bottom, ${test.visuals?.from || '#4F46E5'}, ${test.visuals?.to || '#7C3AED'})` }}></div>
                       ...
                       {/* Highlighted Stats */}
                       <div className="flex flex-wrap md:flex-nowrap items-center justify-start md:justify-end gap-6 lg:gap-12 flex-1 md:pr-6 border-t md:border-t-0 md:border-r border-slate-800/60 pt-4 md:pt-0">
                          {test.isInstance ? (
                             <>
                       ...
                       {/* Action Buttons */}
                       <div className="flex items-center justify-end gap-1 pl-0 md:pl-4 shrink-0 bg-transparent md:bg-[#0B1120] z-10 w-full md:w-auto">
                          {test.isInstance ? (
                             <div className="px-4 py-2 rounded-xl bg-slate-900 border border-slate-800 text-[9px] font-black text-slate-500 uppercase tracking-widest shadow-inner w-full md:w-auto text-center">
                                Managed in Series
                             </div>
                          ) : (
                             <div className="flex items-center gap-1 w-full md:w-auto justify-end">
                               <button onClick={() => triggerFormOpen(test)} className="p-2.5 rounded-xl text-slate-500 hover:text-white hover:bg-slate-800 transition-all" title="Edit Blueprint"><Edit2 size={16}/></button>
                               <button onClick={() => openDeploymentModal(test.id)} className="p-2.5 rounded-xl text-indigo-500 bg-indigo-500/10 hover:text-white hover:bg-indigo-600 transition-all mx-1 shadow-md shadow-indigo-600/10" title="Deploy to Series"><Send size={16}/></button>
                               <button onClick={() => removeExamInstance(test.id)} className="p-2.5 rounded-xl text-slate-500 hover:text-rose-400 hover:bg-rose-500/10 transition-all" title="Delete Blueprint"><Trash2 size={16}/></button>
                               <div className="hidden md:block w-px h-5 bg-slate-800 mx-2"></div>
                               <button onClick={() => navigate(`/forge/mock-exam/${test.id}`)} className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-amber-600 to-orange-600 text-white flex items-center justify-center gap-2 hover:scale-105 transition-all text-[10px] font-black uppercase tracking-widest shadow-lg shadow-amber-600/20" title="Forge Editor"><PenTool size={14}/> Forge</button>
                             </div>
                          )}
                       </div>
                    </div>
                  ...
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 border-t border-slate-800/80 pt-10">
                      ...
                      <div className="flex bg-slate-900 rounded-[1.5rem] p-1.5 border border-slate-800 shrink-0 shadow-inner">
                        <button type="button" onClick={() => setSeriesAssignForm(p => ({ ...p, schedulingMode: 'global' }))} className={`px-6 py-3 h-10 flex items-center rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${seriesAssignForm.schedulingMode === 'global' ? 'bg-amber-500 text-white shadow-lg shadow-amber-500/20' : 'text-slate-500 hover:text-slate-300'}`}>Uniform Schedule</button>
                        <button type="button" onClick={() => setSeriesAssignForm(p => ({ ...p, schedulingMode: 'custom' }))} className={`px-6 py-3 h-10 flex items-center rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${seriesAssignForm.schedulingMode === 'custom' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'text-slate-500 hover:text-slate-300'}`}>Custom Per Series</button>
                      </div>
                    </div>
                    ...
                    {seriesAssignForm.schedulingMode === 'global' ? (
                      <div className="p-8 rounded-[3rem] bg-slate-900/60 border border-slate-800/80 grid grid-cols-1 md:grid-cols-2 gap-8 animate-fadeIn shadow-inner">
                        <div>
                          ...
                          <input type="datetime-local" value={seriesAssignForm.globalSchedule.start} onChange={(e) => configureDistributionSchedules('global', 'start', e.target.value)} className="w-full px-6 h-14 rounded-2xl bg-slate-950 border border-slate-800 text-white font-bold text-sm outline-none focus:border-amber-500 transition-all shadow-inner" />
                        </div>
                        <div>
                          ...
                          <input type="datetime-local" value={seriesAssignForm.globalSchedule.end} onChange={(e) => configureDistributionSchedules('global', 'end', e.target.value)} className="w-full px-6 h-14 rounded-2xl bg-slate-950 border border-slate-800 text-white font-bold text-sm outline-none focus:border-amber-500 transition-all shadow-inner" />
                        </div>
                      </div>
                    ) : (
                            ...
                            <div key={seriesId} className="p-8 rounded-[3rem] bg-slate-900/60 border border-slate-800/80 shadow-inner">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                  ...
                                  <input type="datetime-local" value={seriesAssignForm.customSchedules[seriesId]?.start || ''} onChange={(e) => configureDistributionSchedules('custom', 'start', e.target.value, seriesId)} className="w-full px-6 h-14 rounded-2xl bg-slate-950 border border-slate-800 text-white font-bold text-sm outline-none focus:border-indigo-500 transition-all shadow-inner" />
                                </div>
                                <div>
                                  <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-3 ml-3">Expiration Date</label>
                                  <input type="datetime-local" value={seriesAssignForm.customSchedules[seriesId]?.end || ''} onChange={(e) => configureDistributionSchedules('custom', 'end', e.target.value, seriesId)} className="w-full px-6 h-14 rounded-2xl bg-slate-950 border border-slate-800 text-white font-bold text-sm outline-none focus:border-indigo-500 transition-all shadow-inner" />
                                </div>
                              </div>
                            </div>
                          );
