    ...
    try {
      ...
      try {
        ...
        if (!staffSnap.empty) {
          ...
          // 4. FETCH PENDING ACCESS REQUESTS
      // 4. FETCH PENDING ACCESS REQUESTS (🚨 Fixed to Root Collection Query)
      try {
        const reqQuery = query(
          collection(db, 'exam_access_requests'), // <-- Changed from collectionGroup to collection
          where('ownerId', '==', uid), 
      ...
      setAccessRequests(reqSnaps.docs.map(d => ({ id: d.id, ref: d.ref, ...d.data() })));
          ...
          console.log("AUTHORIZED TEACHERS LOADED:", strictlyTeachers); // 👈 ADD THIS LINE
