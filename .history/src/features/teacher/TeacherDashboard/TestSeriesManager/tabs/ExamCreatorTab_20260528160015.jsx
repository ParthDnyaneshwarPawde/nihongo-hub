...
import { 
  ...
  Send, Lock, Filter, Calendar, LayoutGrid, LayoutList, 
...
export default function ExamCreatorTab() {
  ...
  const initialExamState = {
    ...
    mode: 'auto', // 'auto' | 'manual' -> Determines how the Command Center launches it
  ...
  useEffect(() => {
    const syncDatabase = async () => {
      try {
        ...
        // Fetch User Data for Lead Badge
        const userDoc = await getDoc(doc(db, 'users', user.uid));
        if (userDoc.exists()) {
          setCurrentUserData(userDoc.data());
        }

        // 1. Fetch Standing Blueprints (NO SCHEDULES IN BLUEPRINT)
        const testsSnap = await getDocs(collection(db, 'mock_tests'));
        const exams = testsSnap.docs.map(d => ({
          id: d.id,
          ...d.data(),
          status: 'blueprint', // Simplified status for creator tab
  ...
  const triggerFormOpen = (test = null) => {
    if (test) {
      ...
      setExamForm({
        ...initialExamState,
        collaborators: [] // Exclude current user from this array, they are the 'authorId'
      });
    }
    setTeacherSearch('');
  ...
  const commitDeploymentConfiguration = async () => {
    ...
    try {
      ...
      alert("Exam successfully mapped to the selected Test Series timelines.");
      ...
      <AnimatePresence mode="wait">
        {isExamFormOpen ? (
          ...
          <motion.form 
            ...
            onSubmit={commitFormToDatabase} className="max-w-[1200px] mx-auto w-full space-y-10 pb-32"
          >
                  ...
                  <div className="bg-slate-950/50 p-6 rounded-[2rem] border border-slate-800 shadow-inner flex flex-col space-y-4">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-[10px] text-slate-500 mt-1 font-medium">Deny late joiners after launch.</p>
                      </div>
            ...
            <div className="flex gap-6 sticky bottom-6 z-30 bg-[#090E1A]/90 backdrop-blur-md p-4 rounded-[2.5rem] border border-slate-800/80 shadow-2xl">
              <button type="button" onClick={closeForm} className="w-1/3 py-5 rounded-[2rem] text-sm font-black uppercase tracking-widest border border-slate-800 text-slate-400 hover:bg-slate-800 hover:text-white transition-all duration-200">Discard Blueprint</button>
              <button type="submit" disabled={isSaving} className="flex-1 py-5 bg-white text-black rounded-[2rem] text-sm font-black uppercase tracking-widest flex items-center justify-center gap-3 shadow-[0_10px_30px_rgba(255,255,255,0.1)] transition-all duration-200 active:scale-98">
                {isSaving ? <Loader2 size={16} className="animate-spin" /> : <Save size={18} />} {editingId ? 'Update Blueprint Rules' : 'Finalize & Save Blueprint'}
              </button>
            </div>
          </motion.form>
