...
import { 
  ...
  Globe, Sparkles, Trash2, Loader2, Shield, ArrowRight, BookOpen
} from 'lucide-react';
...
export default function ExamCreatorTab() {
  ...
  // ==========================================
  // UI CLASSES (YOUR EXACT PREMIUM STYLES)
  // ==========================================
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
        ...
        {!isExamFormOpen && (
          <button 
            ...
            className="px-8 py-5 bg-white text-black rounded-[2.5rem] text-sm font-black uppercase tracking-[0.2em] flex items-center gap-3 shadow-xl hover:scale-105 transition-all duration-300 shrink-0"
          >
      ...
      <AnimatePresence mode="wait">
        {isExamFormOpen ? (
          ...
          >
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 animate-in slide-in-from-right-8 duration-500">
              ...
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div className="space-y-3">
                    <label className={labelPremiumClass}>Duration (Minutes)</label>
              ...
              <div className="space-y-8 flex flex-col">
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-slate-950/50 p-6 rounded-[3rem] border border-slate-800 shadow-inner flex flex-col space-y-4">
                    ...
                    {examForm.entryLockout && (
                      <div className="flex items-center gap-4 p-4 rounded-2xl bg-[#0B1120] border border-slate-800 animate-fadeIn">
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 pl-2">Lock Room After:</span>
                        <input type="number" name="lockoutMinutes" value={examForm.lockoutMinutes} onChange={handleFormInput} className="w-20 p-2 rounded-xl bg-[#0D1527] border border-slate-700 text-white text-center font-black outline-none focus:border-rose-500" min="1" />
                    ...
                    {examForm.allowLateAttempts && (
                      <div className="flex items-center gap-4 p-4 rounded-2xl bg-[#0B1120] border border-slate-800 animate-fadeIn mt-2">
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 pl-2">Final Cutoff:</span>
                        <input type="datetime-local" name="lateDeadline" value={examForm.lateDeadline} onChange={handleFormInput} className="flex-1 p-3 rounded-xl bg-[#0D1527] border border-slate-700 text-white text-[10px] outline-none focus:border-emerald-500" />
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="flex gap-4 sticky bottom-6 z-30 bg-[#090E1A]/95 backdrop-blur-md p-3 rounded-3xl border border-slate-800 shadow-2xl max-w-lg mx-auto mt-10">
              <button type="button" onClick={closeForm} className="px-6 py-3 rounded-2xl text-xs font-bold text-slate-400 hover:bg-slate-800 hover:text-white transition-all uppercase tracking-wider">Discard</button>
              <button type="submit" disabled={isSaving} className="flex-1 py-3 bg-white text-black rounded-2xl text-xs font-black uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg transition-all active:scale-95">
          ...
          <motion.div key="grid" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-[#0B1120]/40 p-3 rounded-3xl border border-slate-800/60">
              <div className="flex items-center gap-1.5 p-1 bg-[#090E1A] border border-slate-800 rounded-2xl w-max overflow-x-auto">
                <button className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all shrink-0 bg-[#131C31] text-amber-400 border border-slate-700/80 shadow-md`}>
                  <Filter size={13} /> Active Blueprints Collection
                </button>
              </div>

              {/* Layout Presentation Multi-Toggle Switch */}
              <div className="flex items-center gap-1.5 bg-[#090E1A] border border-slate-800 rounded-2xl p-1 w-max self-end lg:self-auto">
                <button type="button" onClick={() => setViewMode('grid')} className={`p-2.5 rounded-xl transition-all ${viewMode === 'grid' ? 'bg-[#131C31] text-amber-400 border border-slate-700' : 'text-slate-600 hover:text-slate-300'}`}><LayoutGrid size={15}/></button>
                <button type="button" onClick={() => setViewMode('list')} className={`p-2.5 rounded-xl transition-all ${viewMode === 'list' ? 'bg-[#131C31] text-amber-400 border border-slate-700' : 'text-slate-600 hover:text-slate-300'}`}><LayoutList size={15}/></button>
              </div>
            </div>
            ...
            <div className={viewMode === 'grid' ? "grid grid-cols-1 xl:grid-cols-2 gap-6" : "flex flex-col gap-3"}>
              {runtimeViewChunk.length === 0 ? (
                <div className="py-24 border-2 border-dashed border-slate-800/80 rounded-[3rem] bg-[#0B1120]/40 flex flex-col items-center text-center px-4 col-span-full">
                ...
                runtimeViewChunk.map((test) => {
                  ...
                  if (viewMode === 'list') {
                    return (
                      <div key={test.id} className="pl-3 pr-4 py-3 rounded-[2rem] bg-[#0B1120]/90 border border-slate-800/50 hover:border-slate-700/80 transition-all duration-200 flex flex-col md:flex-row items-center justify-between gap-4 group">
                        ...
                        <div className="hidden md:flex items-center justify-between flex-1 px-4 border-x border-slate-800/60 max-w-xl">
                          <div className="flex flex-col">
                            <span className="text-[8px] font-black uppercase tracking-widest text-slate-500">Target Level</span>
                            <span className="text-xs font-black text-indigo-400 tracking-wider mt-0.5">{test.targetLevel || 'N/A'}</span>
                          </div>
                          <div className="flex flex-col">
                            <span className="text-[8px] font-black uppercase tracking-widest text-slate-500">Duration</span>
                            <span className="text-xs font-black text-amber-500 mt-0.5 flex items-center gap-1">
                              <Clock size={10}/> {test.durationMinutes || 0} Mins
                            </span>
                          </div>
                        ...
                        <div className="flex items-center gap-2 w-full md:w-auto shrink-0 justify-end">
                          <button onClick={() => triggerFormOpen(test)} className="p-2.5 rounded-xl hover:bg-slate-800 text-slate-400 hover:text-white transition-colors text-[10px] font-black uppercase tracking-widest" title="Edit Properties"><Edit2 size={13}/></button>
                          <button onClick={() => openDeploymentModal(test.id)} className="p-2.5 rounded-xl hover:bg-slate-800 text-slate-400 hover:text-indigo-400 transition-colors text-[10px] font-black uppercase tracking-widest" title="Deploy Architecture Package"><Send size={13}/></button>
                  ...
                  // --------------------------------------------------------
                  return (
                    <div key={test.id} className="p-6 md:p-8 rounded-[3rem] bg-[#0B1120] border border-slate-800/80 hover:border-slate-700 transition-all duration-300 shadow-2xl flex flex-col justify-between group relative overflow-hidden">
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-8 p-4 rounded-2xl bg-[#0D1527]/60 border border-slate-800/80 relative z-10">
                          <div>
                            <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mb-1 flex items-center gap-1.5"><BookOpen size={10}/> Target Level</p>
                            <p className="text-sm font-black text-indigo-400 tracking-wider">{test.targetLevel || 'N/A'}</p>
                          </div>
                          <div>
                            <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mb-1 flex items-center gap-1.5"><Clock size={10}/> Duration</p>
                            <p className="text-sm font-black text-amber-500 capitalize flex items-center gap-1.5">
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-4 mt-12 pt-6 border-t border-slate-900">
                <button type="button" onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="p-3 rounded-2xl bg-[#0B1120] border border-slate-800 text-slate-400 hover:text-white disabled:opacity-20 transition-all"><ChevronLeft size={16}/></button>
                <div className="flex items-center gap-2">
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map(pageNumber => (
                    <button key={pageNumber} type="button" onClick={() => setCurrentPage(pageNumber)} className={`w-10 h-10 rounded-2xl text-xs font-black transition-all ${currentPage === pageNumber ? 'bg-amber-500 text-white shadow-lg' : 'bg-[#0B1120] border border-slate-800 text-slate-500 hover:border-slate-700'}`}>{pageNumber}</button>
                  ))}
                </div>
                <button type="button" onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="p-3 rounded-2xl bg-[#0B1120] border border-slate-800 text-slate-400 hover:text-white disabled:opacity-20 transition-all"><ChevronRight size={16}/></button>
              </div>
            )}
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {isSeriesModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-md bg-black/60">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-3xl max-h-[85vh] flex flex-col rounded-[3rem] bg-[#0B1120] border border-slate-800 shadow-2xl overflow-hidden z-10">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {testSeriesList.map(series => {
                      ...
                      return (
                        <div key={series.id} onClick={() => processSeriesItemSelect(series.id)} className={`p-5 rounded-3xl border-2 cursor-pointer transition-all duration-200 flex items-center justify-between ${activeSelection ? 'bg-indigo-500/10 border-indigo-500' : 'bg-[#0D1527]/60 border-slate-800 hover:border-slate-700'}`}>
                          <div>
                          ...
                          <div className={`w-5 h-5 rounded-lg flex items-center justify-center border transition-all ${activeSelection ? 'bg-indigo-500 border-indigo-500 text-white' : 'border-slate-700 bg-transparent'}`}>
                    ...
                    {testSeriesList.length === 0 && (
                      <div className="col-span-full py-8 text-center border-2 border-dashed border-slate-800 rounded-[2rem]">
                ...
                {seriesAssignForm.selectedSeries.length > 0 && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-t border-slate-800/80 pt-8">
                      ...
                      <div className="flex bg-[#090E1A] rounded-2xl p-1.5 border border-slate-800 shrink-0">
                        <button type="button" onClick={() => setSeriesAssignForm(p => ({ ...p, schedulingMode: 'global' }))} className={`px-4 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${seriesAssignForm.schedulingMode === 'global' ? 'bg-amber-500 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'}`}>Uniform Schedule</button>
                        <button type="button" onClick={() => setSeriesAssignForm(p => ({ ...p, schedulingMode: 'custom' }))} className={`px-4 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${seriesAssignForm.schedulingMode === 'custom' ? 'bg-indigo-500 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'}`}>Custom Per Series</button>
                      </div>
                    </div>

                    {seriesAssignForm.schedulingMode === 'global' ? (
                      <div className="p-8 rounded-[3rem] bg-[#0D1527]/60 border border-slate-800/80 grid grid-cols-1 md:grid-cols-2 gap-6 animate-fadeIn">
                        <div>
                          <label className="block text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2 ml-2">Global Unlock Clock</label>
                          <input type="datetime-local" value={seriesAssignForm.globalSchedule.start} onChange={(e) => configureDistributionSchedules('global', 'start', e.target.value)} className="w-full p-5 rounded-[2rem] bg-slate-900 border border-slate-800 text-white font-bold text-sm outline-none focus:border-amber-500 transition-all" />
                        </div>
                        <div>
                          ...
                          <input type="datetime-local" value={seriesAssignForm.globalSchedule.end} onChange={(e) => configureDistributionSchedules('global', 'end', e.target.value)} className="w-full p-5 rounded-[2rem] bg-slate-900 border border-slate-800 text-white font-bold text-sm outline-none focus:border-amber-500 transition-all" />
                        </div>
                      </div>
                    ) : (
                      <div className="space-y-4 animate-fadeIn">
                        {seriesAssignForm.selectedSeries.map(seriesId => {
                          ...
                          return (
                            <div key={seriesId} className="p-8 rounded-[3rem] bg-[#0D1527]/60 border border-slate-800/80">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                  ...
                                  <input type="datetime-local" value={seriesAssignForm.customSchedules[seriesId]?.start || ''} onChange={(e) => configureDistributionSchedules('custom', 'start', e.target.value, seriesId)} className="w-full p-4 rounded-2xl bg-slate-900 border border-slate-800 text-white font-bold text-sm outline-none focus:border-indigo-500 transition-all" />
                                </div>
                                <div>
                                  <label className="block text-[9px] font-black uppercase tracking-widest text-slate-500 mb-2 ml-2">Expiration Date</label>
                                  <input type="datetime-local" value={seriesAssignForm.customSchedules[seriesId]?.end || ''} onChange={(e) => configureDistributionSchedules('custom', 'end', e.target.value, seriesId)} className="w-full p-4 rounded-2xl bg-slate-900 border border-slate-800 text-white font-bold text-sm outline-none focus:border-indigo-500 transition-all" />
                                </div>
                              </div>
                            </div>
                          );
