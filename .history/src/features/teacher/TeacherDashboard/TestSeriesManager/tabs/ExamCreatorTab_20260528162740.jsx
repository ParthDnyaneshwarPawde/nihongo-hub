...
import { 
  ...
  Send, Lock, Filter, Activity, Calendar, 
  CheckCircle2, LayoutGrid, LayoutList, ChevronLeft, ChevronRight,
  Globe, Sparkles, Trash2, Loader2, Shield
} from 'lucide-react';
...
export default function ExamCreatorTab() {
  ...
  // 🚨 FIXED: Renamed collabSearch to teacherSearch to match the UI
  ...
  return (
    <div className="flex flex-col h-full max-w-[1200px] mx-auto relative select-none pb-20 pt-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div>
          ...
          <p className="text-sm font-bold text-slate-500 mt-2">Design high-fidelity custom assessment templates and trigger synchronized deployments.</p>
        </div>
        {!isExamFormOpen && (
          <button 
            onClick={() => triggerFormOpen()} 
            className="px-8 py-4 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 text-white rounded-xl text-xs font-black uppercase tracking-[0.1em] flex items-center gap-2 shadow-xl shadow-amber-600/20 transition-all duration-200 active:scale-95 shrink-0"
          >
      ...
      <AnimatePresence mode="wait">
        {isExamFormOpen ? (
          ...
          <motion.form 
            ...
            onSubmit={commitFormToDatabase} className="w-full pb-32"
          >
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 animate-in slide-in-from-right-8 duration-500">
              ...
              <div className="space-y-8">
                <div className="space-y-3">
                  <label className={labelPremiumClass}>
                    Product Title {editingId && <span className="text-rose-500 ml-2">(LOCKED)</span>}
                  </label>
                  <input 
                    required 
                    name="title"
                ...
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <button 
                      ...
                      className={`w-full py-6 rounded-[2rem] font-black text-xs transition-all shadow-xl tracking-widest uppercase flex items-center justify-center gap-2 ${examForm.mode === 'auto' ? 'bg-amber-600 text-white shadow-amber-500/20' : 'bg-indigo-600 text-white shadow-indigo-500/20'}`}
                    >
              ...
              <div className="space-y-8 flex flex-col">
                <div className="space-y-3 flex-1 flex flex-col">
                  <label className={labelPremiumClass}>Collaborators</label>
            ...
            {/* Smaller, Centered Action Buttons at Footer */}
            <div className="flex gap-4 sticky bottom-6 z-30 bg-[#090E1A]/95 backdrop-blur-md p-3 rounded-2xl border border-slate-800 shadow-2xl max-w-lg mx-auto mt-10">
              <button type="button" onClick={closeForm} className="px-6 py-2.5 rounded-xl text-xs font-bold text-slate-400 hover:bg-slate-800 hover:text-white transition-all uppercase tracking-wider">Discard</button>
            ...
            <div className={viewMode === 'grid' ? "grid grid-cols-1 xl:grid-cols-2 gap-6" : "flex flex-col gap-3"}>
              {runtimeViewChunk.length === 0 ? (
                ...
                runtimeViewChunk.map((test) => {
                  ...
                  // --------------------------------------------------------
                  // EXACT Match List View CSS
                  // --------------------------------------------------------
                  if (viewMode === 'list') {
                    return (
                      <div key={test.id} className="flex items-center justify-between p-5 rounded-2xl border border-slate-800 bg-transparent group">
                         <div className="flex items-center gap-4 w-[30%]">
                            <div className="w-1.5 h-10 rounded-full" style={{ background: `linear-gradient(to bottom, ${test.visuals?.from || '#4F46E5'}, ${test.visuals?.to || '#7C3AED'})` }}></div>
                            <div>
                               <h4 className="text-white font-black text-sm truncate">{test.title}</h4>
                               <div className="flex gap-2 items-center mt-1">
                                  <span className="px-1.5 py-0.5 rounded text-[8px] font-black bg-slate-800 text-slate-400 border border-slate-700">BLUEPRINT</span>
                                  <span className="text-[10px] text-slate-500 font-mono truncate">{test.id}</span>
                               </div>
                            </div>
                         </div>

                         <div className="flex items-center gap-10 flex-1 px-8">
                            <div>
                               <p className="text-[8px] font-black uppercase text-slate-500 tracking-[0.2em] mb-1">Access Token</p>
                               <p className="text-xs font-black text-amber-500">{test.accessKey || 'UNSET'}</p>
                            </div>
                            <div>
                               <p className="text-[8px] font-black uppercase text-slate-500 tracking-[0.2em] mb-1">Launch Logic</p>
                               <p className="text-xs font-black text-white capitalize">{test.mode}</p>
                            </div>
                            <div>
                               <p className="text-[8px] font-black uppercase text-slate-500 tracking-[0.2em] mb-1">Staff</p>
                               <div className="w-6 h-6 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-[8px] font-black text-white">L</div>
                            </div>
                         </div>

                         <div className="flex items-center gap-2">
                            <div className="flex items-center gap-1">
                               <button onClick={() => triggerFormOpen(test)} className="w-9 h-9 rounded-full flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-800 transition-colors" title="Edit"><Edit2 size={14}/></button>
                               <button onClick={() => openDeploymentModal(test.id)} className="w-9 h-9 rounded-full flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-800 transition-colors" title="Deploy"><Send size={14}/></button>
                               <button onClick={() => removeExamInstance(test.id)} className="w-9 h-9 rounded-full flex items-center justify-center text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 transition-colors" title="Drop"><Trash2 size={14}/></button>
                               <button onClick={() => alert("Launching Forge Architect...")} className="w-9 h-9 rounded-xl bg-amber-500/10 text-amber-500 flex items-center justify-center hover:bg-amber-500 hover:text-white transition-colors ml-1" title="Forge"><PenTool size={14}/></button>
                            </div>
                         </div>
                      </div>
                    );
                  ...
                  // --------------------------------------------------------
                  // PRESENTATION SCHEME B: Grid View
                  // --------------------------------------------------------
                  return (
