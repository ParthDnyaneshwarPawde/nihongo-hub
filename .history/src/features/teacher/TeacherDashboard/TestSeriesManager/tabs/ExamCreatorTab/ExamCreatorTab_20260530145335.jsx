      ...
      if (series.tests && Array.isArray(series.tests)) {
        series.tests.forEach(deployment => {
          ...
          if (blueprint) {
            ...
            // Handle both legacy date keys and the new Engine Config keys
            ...
            // 🚨 NEW LOGIC: Respect the Matrix Control Panel's manual status override first!
            if (deployment.statusOverride) {
              if (deployment.statusOverride === 'live') currentStatus = 'live';
              else if (deployment.statusOverride === 'archived') currentStatus = 'over';
              else currentStatus = 'upcoming'; // 'staged'
            } 
            // Fallback to automatic clock calculation if no override exists
            else {
              ...
              if (!hasStart && !hasEnd) {
                ...
                if (now > end) currentStatus = 'over';
                else if (now >= start && now <= end) currentStatus = 'live'; 
      ...
      <AnimatePresence>
        {isRequestsModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 backdrop-blur-xl bg-black/60">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full max-w-2xl bg-[#0B1120] border border-slate-800 rounded-[3rem] shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
              <div className="p-8 border-b border-slate-800 bg-slate-900/50 flex justify-between items-center">
                <div>
                  <h3 className="text-2xl font-black text-white flex items-center gap-3"><Bell className="text-indigo-500"/> Incoming Access Requests</h3>
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mt-1">Teachers requesting access to your Blueprints</p>
                </div>
                <button onClick={() => setIsRequestsModalOpen(false)} className="w-10 h-10 bg-slate-800 rounded-full flex items-center justify-center hover:bg-rose-500 hover:text-white"><X size={16}/></button>
              </div>
              <div className="p-8 overflow-y-auto flex-1 space-y-4">
                {accessRequests.length === 0 ? (
                  ...
                  accessRequests.map(req => (
                    <div key={req.id} className="p-6 rounded-[2rem] bg-slate-900/50 border border-slate-800 flex items-center justify-between">
                      <div>
                        <p className="text-sm font-black text-white"><span className="text-indigo-400">{req.requesterName}</span> wants to use:</p>
                        <p className="text-xl font-black text-white mt-1">{req.testTitle}</p>
                        <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mt-2">Target Series: {req.seriesTitle}</p>
                      </div>
