...
export default function MockForge() {
  ...
  // exerciseId here acts as the global mock test ID
  const { exerciseId } = useParams();
  const isDarkMode = true; // Hardcoded to premium dark mode
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        // Editing Mode: Strict Global Collaborator Bouncer
        if (exerciseId) {
          ...
          if (examSnap.exists()) {
            ...
            // Authorized! Load the architecture
            setExamSettings({
              title: data.title || '',
    ...
    try {
      ...
      parts.forEach((part) => {
        ...
        part.sections.forEach((section) => {
          ...
          activeQs.forEach((q) => {
            // 🚨 8-Digit Padded Global Question ID Logic
      ...
      // 🚨 Save to Global Mock Tests Collection
      const examRef = exerciseId 
        ? doc(db, 'mock_tests', exerciseId) 
        : doc(collection(db, 'mock_tests'));
      ...
      batch.set(examRef, { 
        ...
        collaborators: exerciseId ? undefined : [actualTeacherId], // Only set initial creator on creation
    ...
    <div className="min-h-screen bg-[#0B1121] text-slate-200 pb-32 transition-colors">
      <header className="sticky top-0 z-50 px-6 py-4 border-b flex items-center justify-between backdrop-blur-xl bg-[#0B1121]/80 border-slate-800 shadow-xl">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-black flex items-center gap-2"><Sparkles size={18} className="text-rose-500" /> Advanced Blueprint Forge</h1>
            <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">{exerciseId ? 'Editing Global Architecture' : 'New 3-Tier Assessment'}</p>
          </div>
        </div>
