...
import { 
  ...
  Layers, Clock, Plus, Trash2, ChevronDown, ChevronUp 
} from 'lucide-react';
...
// Removed allowSecondAttempt and hint
const generateBlankQuestion = () => ({ 
  ...
  gradingRubric: false, afterTimeHint: false, audioPlaybackType: 'R',
...
export default function MockForge() {
  ...
  const handleSaveQuiz = async () => {
    ...
    // 🚨 DURATION VALIDATION (Crucial addition)
    ...
    if (sumOfParts !== Number(examSettings.durationMinutes)) {
      alert(`⚠️ Duration Mismatch Warning!\n\nYour Global Exam Duration is set to ${examSettings.durationMinutes} mins.\nHowever, the time limits of your Individual Parts add up to ${sumOfParts} mins.\n\nPlease adjust them so they match perfectly before saving.`);
      return;
    }
    ...
    try {
        ...
        part.sections.forEach((section) => {
          ...
          activeQs.forEach((q) => {
            ...
            batch.set(doc(db, `question_bank/global_mocks/questions`, finalQId), {
              ...
              topic: q.topic, subTopic: q.subTopic, gradingRubric: q.gradingRubric, 
              afterTimeHint: q.afterTimeHint, audioPlaybackType: q.audioPlaybackType,
          ...
          sectionPayloads.push({
            ...
            conditionalNavigation: section.conditionalNavigation, questionIds: questionIdPayloads
          });
        });

        structuredPartsPayload.push({
          id: part.id, title: part.title, timeLimitMinutes: Number(part.timeLimitMinutes), sections: sectionPayloads
        });
      });
      ...
      batch.set(examRef, { 
        ...
        collaborators: exerciseId ? undefined : [actualTeacherId], 
  ...
  if (accessDenied) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center bg-[#0B1121] text-slate-200">
        ...
        <p className="text-sm font-medium text-slate-500 max-w-md leading-relaxed">You are not a registered collaborator for this Blueprint, so you cannot edit the forge.</p>
              ...
              <motion.div key={part.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-[#151E2E] rounded-[2rem] md:rounded-[3rem] border border-slate-800/80 overflow-hidden shadow-2xl relative">
                ...
                <div className="bg-slate-900/90 backdrop-blur-md p-6 md:p-8 border-b border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                  <div className="flex-1 flex flex-col gap-5 w-full">
                     <div className="flex items-center gap-4">
                        <span className="bg-indigo-500/10 text-indigo-400 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest border border-indigo-500/20 shadow-inner">Part {pIdx + 1}</span>
                     ...
                     <div className="flex items-center gap-3 bg-[#0B1121] w-fit p-1.5 pr-4 rounded-xl border border-slate-800">
                        ...
                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em]">Minutes</span>
                     </div>
                  </div>
                           ...
                           <div className="flex items-start justify-between gap-4 mb-8">
                              <div className="flex-1 space-y-5">
                                 ...
                                 <div className="flex flex-col sm:flex-row sm:items-center gap-6 pt-2">
                                    <div className="flex items-center gap-3 bg-emerald-500/5 p-2 pr-4 rounded-xl border border-emerald-500/10">
