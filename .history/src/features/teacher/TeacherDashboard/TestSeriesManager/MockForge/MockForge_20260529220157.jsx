...
import { 
  ...
  Layers, Clock, Coffee, Plus, Trash2, ChevronDown, ChevronUp, Share2 
} from 'lucide-react';
...
// 🚨 UPDATED: Section now has Pass/Fail targets for "Early Exit" routing
...
// Part has Pass/Fail targets for "Macro" routing
...
export default function MockForge() {
  ...
  // ==========================================
  // BULLETPROOF STATE MUTATION HANDLERS
  // ==========================================
  ...
  // ==========================================
  // GRAPH TRAVERSAL: Advanced DAG Engine
  // ==========================================
  const validatePaths = () => {
    ...
    // 1. Detect Infinite Loops (Cycles) using Depth First Search
    const checkCycles = (currentId, visited) => {
       if (visited.has(currentId)) return true; // Found a loop!
       const part = partMap[currentId];
       ...
       if (part.conditionalNavigation) {
          if (part.branchTargetIdPass && part.branchTargetIdPass !== 'END') hasCycle = hasCycle || checkCycles(part.branchTargetIdPass, new Set(visited));
          if (part.branchTargetIdFail && part.branchTargetIdFail !== 'END') hasCycle = hasCycle || checkCycles(part.branchTargetIdFail, new Set(visited));
       } else {
          const currentIndex = parts.findIndex(p => p.id === currentId);
          if (currentIndex !== -1 && currentIndex < parts.length - 1) {
             hasCycle = hasCycle || checkCycles(parts[currentIndex + 1].id, new Set(visited));
          }
       }

       // Check Section Early Exits
       (part.sections || []).forEach(sec => {
          if (sec.conditionalNavigation) {
             if (sec.branchTargetIdPass && sec.branchTargetIdPass !== 'END') hasCycle = hasCycle || checkCycles(sec.branchTargetIdPass, new Set(visited));
             if (sec.branchTargetIdFail && sec.branchTargetIdFail !== 'END') hasCycle = hasCycle || checkCycles(sec.branchTargetIdFail, new Set(visited));
          }
       });
    ...
    if (checkCycles(parts[0].id, new Set())) {
      return { valid: false, message: "⚠️ Infinite Loop Detected!\n\nYour Branching Logic routes a student back to a Part they have already taken. Standardized adaptive engines must flow forward (DAG)." };
    }

    // 2. Calculate Longest Possible Path Duration
    const getLongestPath = (currentId) => {
       const part = partMap[currentId];
       if (!part) return 0;
       
       const timeInThisPart = Number(part.timeLimitMinutes || 0);
       let maxSubsequentTime = 0;

       // Evaluate Normal Part-Level Paths
       if (part.conditionalNavigation) {
          const passTime = (part.branchTargetIdPass && part.branchTargetIdPass !== 'END') ? getLongestPath(part.branchTargetIdPass) : 0;
          const failTime = (part.branchTargetIdFail && part.branchTargetIdFail !== 'END') ? getLongestPath(part.branchTargetIdFail) : 0;
          maxSubsequentTime = Math.max(maxSubsequentTime, passTime, failTime);
       } else {
          const currentIndex = parts.findIndex(p => p.id === currentId);
          if (currentIndex !== -1 && currentIndex < parts.length - 1) {
              maxSubsequentTime = Math.max(maxSubsequentTime, getLongestPath(parts[currentIndex + 1].id));
          }
       }

       // Evaluate Section-Level (Early Exit) Paths
       (part.sections || []).forEach(sec => {
          if (sec.conditionalNavigation) {
             const secPassTime = (sec.branchTargetIdPass && sec.branchTargetIdPass !== 'END') ? getLongestPath(sec.branchTargetIdPass) : 0;
             const secFailTime = (sec.branchTargetIdFail && sec.branchTargetIdFail !== 'END') ? getLongestPath(sec.branchTargetIdFail) : 0;
             maxSubsequentTime = Math.max(maxSubsequentTime, secPassTime, secFailTime);
          }
       });

       return timeInThisPart + maxSubsequentTime;
    };

    const longestPathDuration = getLongestPath(parts[0].id);

    if (longestPathDuration !== Number(examSettings.durationMinutes)) {
       return { valid: false, message: `⚠️ Path Duration Mismatch!\n\nYour Global Exam Duration is ${examSettings.durationMinutes} mins.\nHowever, based on your interconnected Branching Paths (both Part & Section early exits), the longest route a student can take is exactly ${longestPathDuration} mins.\n\nPlease ensure your Global Duration matches the actual mathematical length of your paths.` };
    }
  ...
  const handleSaveQuiz = async () => {
    ...
    // 🚨 Run the Graph Traversal Validation
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 mt-12 space-y-12">
        ...
        <section className="space-y-10">
          <div className="flex items-end justify-between border-b border-slate-800 pb-4">
            <div>
               <h2 className="text-2xl font-black text-white flex items-center gap-3 tracking-tight"><Share2 className="text-indigo-500"/> Structural Hierarchy</h2>
          ...
          <AnimatePresence>
            {parts.map((part, pIdx) => (
              <motion.div key={part.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-[#151E2E] rounded-[2rem] md:rounded-[3rem] border border-slate-800/80 overflow-hidden shadow-2xl relative">
                ...
                <div className="bg-slate-900/90 backdrop-blur-md p-6 md:p-8 border-b border-slate-800 flex flex-col md:flex-row md:items-start justify-between gap-6 relative z-10">
                  <div className="flex-1 flex flex-col gap-5 w-full">
                     <div className="flex flex-col md:flex-row md:items-center gap-4">
                        <span className="bg-indigo-500/10 text-indigo-400 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest border border-indigo-500/20 shadow-inner w-fit shrink-0">Part {pIdx + 1}</span>
                     ...
                     <div className="flex flex-wrap items-center gap-3">
                        ...
                        <div className="flex items-center gap-3 bg-indigo-500/5 p-1.5 pr-3 rounded-xl border border-indigo-500/10 w-fit">
                           <span className="text-[9px] font-black uppercase tracking-widest text-indigo-400 px-2">Part Pass %</span>
                           <input type="number" value={part.passPercentage || ''} onChange={(e) => updatePartField(part.id, 'passPercentage', e.target.value)} className="w-12 bg-transparent text-center text-xs font-black text-indigo-400 outline-none"/>
                        </div>
                        <label className="flex items-center gap-3 cursor-pointer group" onClick={() => updatePartField(part.id, 'conditionalNavigation', !part.conditionalNavigation)}>
                           <div className={`relative w-10 h-5 rounded-full transition-colors shrink-0 ${part.conditionalNavigation ? 'bg-amber-500' : 'bg-slate-700'}`}>
                              <div className={`absolute top-[2px] w-4 h-4 rounded-full bg-white transition-transform ${part.conditionalNavigation ? 'translate-x-5' : 'left-[2px]'}`}></div>
                           </div>
                           <span className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 group-hover:text-slate-300 transition-colors">Branching</span>
                        </label>
                     </div>
                     
                     {part.conditionalNavigation && (
                        <div className="flex flex-col sm:flex-row gap-3 pt-2 animate-in fade-in">
                           <div className="flex items-center gap-3 bg-emerald-500/5 p-1.5 rounded-xl border border-emerald-500/20 w-fit">
                              <span className="text-[9px] font-black uppercase tracking-[0.1em] text-emerald-500 px-2">If Pass, Skip To:</span>
                              <select value={part.branchTargetIdPass || ''} onChange={(e) => updatePartField(part.id, 'branchTargetIdPass', e.target.value)} className="bg-transparent text-xs font-black text-emerald-400 outline-none cursor-pointer pr-2">
                                 <option value="">Next Part (Linear)</option>
                                 <option value="END">End Exam</option>
                                 {parts.map((p, i) => p.id !== part.id && <option key={`pass_${p.id}`} value={p.id}>Part {i+1}</option>)}
                              </select>
                           </div>
                           <div className="flex items-center gap-3 bg-rose-500/5 p-1.5 rounded-xl border border-rose-500/20 w-fit">
                              <span className="text-[9px] font-black uppercase tracking-[0.1em] text-rose-500 px-2">If Fail, Skip To:</span>
                              <select value={part.branchTargetIdFail || ''} onChange={(e) => updatePartField(part.id, 'branchTargetIdFail', e.target.value)} className="bg-transparent text-xs font-black text-rose-400 outline-none cursor-pointer pr-2">
                                 <option value="">Next Part (Linear)</option>
                                 <option value="END">End Exam</option>
                                 {parts.map((p, i) => p.id !== part.id && <option key={`fail_${p.id}`} value={p.id}>Part {i+1}</option>)}
                              </select>
                           </div>
                        </div>
                     )}
                              ...
                              <div className="flex-1 space-y-5">
                                 ...
                                 <div className="flex flex-col md:flex-row md:items-center gap-6 pt-2">
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                                       <label className="flex items-center gap-3 cursor-pointer group" onClick={() => updateSectionField(part.id, section.id, 'conditionalNavigation', !section.conditionalNavigation)}>
                                          ...
                                          <span className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 group-hover:text-slate-300 transition-colors">Early Exit Branching</span>
                                       </label>
                                    </div>
                                 </div>
                                 
                                 {/* 🚨 UPDATED: Display both Target Pass and Target Fail for Sections */}
                                 {section.conditionalNavigation && (
                                    <div className="flex flex-col sm:flex-row gap-3 pt-2 animate-in fade-in">
                                       <div className="flex items-center gap-3 bg-emerald-500/5 p-1.5 rounded-xl border border-emerald-500/20 w-fit">
                                          <span className="text-[9px] font-black uppercase tracking-[0.1em] text-emerald-500 px-2">If Pass, Skip To:</span>
                                          <select value={section.branchTargetIdPass || ''} onChange={(e) => updateSectionField(part.id, section.id, 'branchTargetIdPass', e.target.value)} className="bg-transparent text-xs font-black text-emerald-400 outline-none cursor-pointer pr-2">
                                             <option value="">Next Section/Part</option>
                                             <option value="END">End Exam</option>
                                             {parts.map((p, i) => p.id !== part.id && <option key={`sec_pass_${p.id}`} value={p.id}>Part {i+1}</option>)}
                                          </select>
                                       </div>
                                       <div className="flex items-center gap-3 bg-rose-500/5 p-1.5 rounded-xl border border-rose-500/20 w-fit">
                                          <span className="text-[9px] font-black uppercase tracking-[0.1em] text-rose-500 px-2">If Fail, Skip To:</span>
                                          <select value={section.branchTargetIdFail || ''} onChange={(e) => updateSectionField(part.id, section.id, 'branchTargetIdFail', e.target.value)} className="bg-transparent text-xs font-black text-rose-400 outline-none cursor-pointer pr-2">
                                             <option value="">Next Section/Part</option>
                                             <option value="END">End Exam</option>
                                             {parts.map((p, i) => p.id !== part.id && <option key={`sec_fail_${p.id}`} value={p.id}>Part {i+1}</option>)}
                                          </select>
                                       </div>
                                    </div>
                                 )}
