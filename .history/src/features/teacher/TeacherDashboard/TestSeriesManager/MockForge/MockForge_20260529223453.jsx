            ...
            {parts.map((part, pIdx) => (
              <motion.div key={part.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-[#151E2E] rounded-[2rem] md:rounded-[3rem] border border-slate-800/80 overflow-hidden shadow-2xl relative">
                ...
                <div className="bg-slate-900/90 backdrop-blur-md p-6 md:p-8 border-b border-slate-800 relative z-10">
                  <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                    <div className="flex-1 flex flex-col gap-4 w-full">
                       <div className="flex items-center gap-4 w-full">
                          ...
                          <input type="text" value={part.title || ''} onChange={(e) => updatePartField(part.id, 'title', e.target.value)} placeholder="e.g. Vocabulary & Grammar" className="bg-transparent border-none outline-none font-black text-xl md:text-2xl text-white placeholder:text-slate-600 w-full min-w-0" />
                       </div>
                       
                       {/* Aligned Part Controls Grid */}
                       <div className="flex flex-wrap items-center gap-3">
                          <div className="flex items-center gap-2 bg-[#0B1121] h-10 px-3 rounded-xl border border-slate-800">
                             <Clock size={16} className="text-amber-500"/>
                             <input type="number" value={part.timeLimitMinutes || ''} onChange={(e) => updatePartField(part.id, 'timeLimitMinutes', e.target.value)} className="w-12 bg-transparent text-center text-sm font-black text-amber-400 outline-none"/>
                             <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Working Mins</span>
                          </div>
                          <div className="flex items-center gap-2 bg-[#0B1121] h-10 px-3 rounded-xl border border-slate-800">
                             <Coffee size={16} className="text-emerald-500"/>
                             <input type="number" value={part.breakAfterMinutes || ''} onChange={(e) => updatePartField(part.id, 'breakAfterMinutes', e.target.value)} className="w-12 bg-transparent text-center text-sm font-black text-emerald-400 outline-none"/>
                             <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Break Mins</span>
                          </div>
                          <div className="flex items-center gap-2 bg-indigo-500/5 h-10 px-3 rounded-xl border border-indigo-500/10">
                             <span className="text-[9px] font-black uppercase tracking-widest text-indigo-400">Pass %</span>
                             <input type="number" value={part.passPercentage || ''} onChange={(e) => updatePartField(part.id, 'passPercentage', e.target.value)} className="w-10 bg-transparent text-center text-sm font-black text-indigo-400 outline-none"/>
                          </div>
                          <label className="flex items-center gap-2 bg-[#0B1121] h-10 px-3 rounded-xl border border-slate-800 cursor-pointer group" onClick={() => updatePartField(part.id, 'conditionalNavigation', !part.conditionalNavigation)}>
                             <div className={`relative w-8 h-4 rounded-full transition-colors shrink-0 ${part.conditionalNavigation ? 'bg-amber-500' : 'bg-slate-700'}`}>
                             ...
                             <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 group-hover:text-slate-300 transition-colors">Branching</span>
                          </label>
                       </div>
                    </div>
                  ...
                  {/* Clean Branching Dropdowns */}
                  {part.conditionalNavigation && (
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6 p-5 rounded-2xl bg-[#0B1121]/50 border border-slate-800/50 animate-in fade-in">
                        <div className="flex flex-col gap-2">
                           <label className="text-[9px] font-black uppercase tracking-widest text-emerald-500 flex items-center gap-1.5">✅ If Passed, Skip To:</label>
                           <select value={part.branchTargetIdPass || ''} onChange={(e) => updatePartField(part.id, 'branchTargetIdPass', e.target.value)} className="h-11 w-full px-4 rounded-xl bg-[#151E2E] border border-emerald-500/20 text-xs font-bold text-emerald-400 outline-none focus:border-emerald-500 cursor-pointer">
                              {renderTargetOptions(part.id)}
                           </select>
                        </div>
                        <div className="flex flex-col gap-2">
                           <label className="text-[9px] font-black uppercase tracking-widest text-rose-500 flex items-center gap-1.5">❌ If Failed, Skip To:</label>
                           <select value={part.branchTargetIdFail || ''} onChange={(e) => updatePartField(part.id, 'branchTargetIdFail', e.target.value)} className="h-11 w-full px-4 rounded-xl bg-[#151E2E] border border-rose-500/20 text-xs font-bold text-rose-400 outline-none focus:border-rose-500 cursor-pointer">
                              {renderTargetOptions(part.id)}
                           </select>
                        </div>
                     </div>
                        ...
                        <div key={section.id} className="bg-slate-900/60 rounded-[2rem] border border-slate-700/50 p-5 md:p-8 shadow-inner">
                           <div className="flex items-start justify-between gap-4 mb-6">
                              <div className="flex-1 space-y-4">
                                 {/* 🚨 FIXED: Straight Line Title Alignment */}
                                 <div className="flex items-center gap-4 w-full">
                                    <span className="text-slate-500 font-black text-[11px] uppercase tracking-[0.2em] shrink-0 whitespace-nowrap">Section {sIdx + 1}</span>
                                    <input type="text" value={section.title || ''} onChange={(e) => updateSectionField(part.id, section.id, 'title', e.target.value)} placeholder="Section Title..." className="bg-transparent border-none outline-none font-black text-lg text-indigo-300 w-full min-w-0" />
                                 </div>
                                 ...
                                 <div className="flex flex-wrap items-center gap-3 pt-2">
                                    <div className="flex items-center gap-2 bg-emerald-500/5 h-10 px-3 rounded-xl border border-emerald-500/10">
                                       <span className="text-[9px] font-black uppercase tracking-widest text-emerald-500">Pass %</span>
                                       <input type="number" value={section.passPercentage || ''} onChange={(e) => updateSectionField(part.id, section.id, 'passPercentage', e.target.value)} className="w-10 bg-transparent text-center text-sm font-black text-emerald-400 outline-none"/>
                                    </div>
                                    <label className="flex items-center gap-2 bg-[#0B1121] h-10 px-3 rounded-xl border border-slate-800 cursor-pointer group" onClick={() => updateSectionField(part.id, section.id, 'conditionalNavigation', !section.conditionalNavigation)}>
                                       <div className={`relative w-8 h-4 rounded-full transition-colors shrink-0 ${section.conditionalNavigation ? 'bg-amber-500' : 'bg-slate-700'}`}>
                                          <div className={`absolute top-[2px] w-3 h-3 rounded-full bg-white transition-transform ${section.conditionalNavigation ? 'translate-x-4' : 'left-[2px]'}`}></div>
                                       </div>
                                       <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 group-hover:text-slate-300 transition-colors">Early Exit Branching</span>
                                    </label>
                                 </div>
                              </div>
                           ...
                           {section.conditionalNavigation && (
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6 p-5 rounded-2xl bg-[#0B1121]/30 border border-slate-800/50 animate-in fade-in">
                                 <div className="flex flex-col gap-2">
                                    <label className="text-[9px] font-black uppercase tracking-widest text-emerald-500 flex items-center gap-1.5">✅ If Passed, Skip To:</label>
                                    <select value={section.branchTargetIdPass || ''} onChange={(e) => updateSectionField(part.id, section.id, 'branchTargetIdPass', e.target.value)} className="h-11 w-full px-4 rounded-xl bg-[#151E2E] border border-emerald-500/20 text-xs font-bold text-emerald-400 outline-none focus:border-emerald-500 cursor-pointer">
                                       {renderTargetOptions(section.id)}
                                    </select>
                                 </div>
                                 <div className="flex flex-col gap-2">
                                    <label className="text-[9px] font-black uppercase tracking-widest text-rose-500 flex items-center gap-1.5">❌ If Failed, Skip To:</label>
                                    <select value={section.branchTargetIdFail || ''} onChange={(e) => updateSectionField(part.id, section.id, 'branchTargetIdFail', e.target.value)} className="h-11 w-full px-4 rounded-xl bg-[#151E2E] border border-rose-500/20 text-xs font-bold text-rose-400 outline-none focus:border-rose-500 cursor-pointer">
                                       {renderTargetOptions(section.id)}
                                    </select>
                                 </div>
                              </div>
